...
- **Markdown の節を消すスクリプトを「次の `###` まで」で区切ってはいけない。**
その節が `## ` の直前にあると、**`##` の見出しとそれ以降を丸ごと巻き込む**。
実測で `docs/backlog.md` の `## Phase 4` と `## 意図的に見送っているもの` を
消してしまい、**push するまで気づかなかった**(2026-09-13、`P3-08`)。
検算が「消したい節が 1 つになったか」だけだったのが原因である。
**`^#{1,3} ` で区切り、消す前後で見出しの一覧を `diff` すること。**
(なお、同じ節が新旧で二重に残る事故は `P2A-20` でも起きている。
**状態を書き換えるときは「古い節を消す」より「古い節を書き換える」ほうが安全。**)
...
- **PostgreSQL の照合順序はイメージで変わる。`ORDER BY` は Python の
  ...
  **ラベルや説明には使わない** — 日本語の並びが壊れる。
  ...
  (`P3-12`)
- **GitHub Actions に浮動メジャータグがあるとは限らない。** `@v10` のような
  書き方は、そのリポジトリがメジャータグを動かしている場合にしか通らない。
  **実測で `astral-sh/setup-uv` は `v1`〜`v7` しか持たず、最新リリースが
  `v10.1.0` でも `v10` は存在しない**(ブランチにも無い)。書くと
  `Unable to resolve action ... unable to find version` で**全ジョブが
  1〜3 秒で落ちる**。`gh api repos/<owner>/<repo>/git/matching-refs/tags/v`
  で**実在するタグを確かめてから**書く。無ければ正確なタグを書く
  (`@v10.1.0`)。**「最新リリースがあるなら浮動タグもある」は成り立たない。**
