...
**下の表は「こちらの判断だけでは進めないもの」である**（Phase 3 の残り`P3-02`〜`P3-04` / `P3-06` / `P3-09` と Phase 4 は別表にある）。`P2A-21` と `P3-07` は**ローカルで検証できない**（デプロイ窓が必要）、`P2A-22` は**消してよいことを確かめていない**。どちらも、判断だけで進めると「緑なのに何も検証していない」状態を作る。
