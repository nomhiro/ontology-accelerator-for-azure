...
最終更新: 2026-09-13（**Phase 2 が完了した**。`P2A-20`（Web の MSAL 認証）まで実装済みで、残るのは `P2A-21`（デプロイ済みオリジンでのサインイン。要デプロイ窓）と Phase 3 である）
...
**柱 A の残りは 2 件になった。** `P2A-21` は**ローカルで検証できない**（デプロイ窓が必要）、`P2A-22` は**消してよいことを確かめていない**。どちらも、こちらの判断だけで進めると「緑なのに何も検証していない」状態を作る。
...
**次に窓が必要なのは `P2A-21` だけである。** Static Web Apps のホスト名が
provision 後にしか分からないため。ついでに確かめられることを下にまとめておく
（窓は高いので 1 回で取りきる）。
