...
**残りはすべて「ローカルで検証できない」か「人の好みが入る」ものである。** どちらも、こちらの判断だけで進めると「緑なのに何も検証していない」状態を作る。

| ID | 内容 | 何が要るか |
|---|---|---|
| `P2A-02` | LLM によるオントロジー候補生成（OWL/SHACL） | **実装は完了（2026-09-12、[ADR-0043](adr/0043-ontology-proposal.md)）。残るのは実機確認 — プロンプトの品質は実モデルでしか測れない** |
| `P2A-19` | スキャンの定期実行（ACA Job） | **実装は完了（2026-09-12、[ADR-0042](adr/0042-scan-job.md)）。残るのは実機確認だけ** |
| `P2A-09` | `platform-admin` アプリロールの**実機確認** | **デプロイ窓（課金）。** 自動化は完了、実 Entra テナントに対しては未実行 |
...
**デプロイ窓では 3 件をまとめて確かめる**（`P2A-09` / `P2A-18` / `P2A-19`）。窓を開けるのは約 11 分の `azd up` と約 24 分の `azd down --purge` を伴うので、1 回で取りきる。確かめる項目は次のとおり。

| 何を | どのタスク |
|---|---|
| アプリロールの定義（Graph への PATCH）と割り当てが通るか | `P2A-09` |
| 実トークンの `roles` クレームに `platform-admin` が乗るか | `P2A-09` |
| `platform-admin` を持たない主体が 403 になるか | `P2A-09` |
| `platform-admin` が四眼原則を**飛び越えられない**か（不変条件12） | `P2A-09` |
| ~~`idtyp: "user"` が v2.0 のアクセストークンに乗るか~~ → **乗った**（2026-09-13） | `P2A-18` |
| ACA Job が起動し、`Succeeded` で終わるか | `P2A-19` |
| `postdeploy` がジョブのイメージを差し替えるか（azd が更新しないため） | `P2A-19` |
| マネージド ID で PostgreSQL とソース DB に届くか | `P2A-19` |
| `SCAN_JOB_CRON` を設定すると `Schedule` トリガになるか | `P2A-19` |
| モデルのデプロイが作られるか（割り当て・SKU） | `P2A-02` |
| マネージド ID で推論が呼べるか（キー認証は無効） | `P2A-02` |
| **実モデルが検証を通る Turtle を返すか**（試行回数） | `P2A-02` |

**自動化とローカルの検証は済んでいる**（`just setup-app-role` と `preprovision` のゲート。2026-09-09）。残っているのは**実 Entra テナントと実デプロイに対して一度も実行していない**という一点である。
...
### `P2A-20` Web の認証を MSAL にする

- **状態**: 未着手（**要デプロイ窓**）
- **優先**: 中
- **内容**: 画面が `@azure/msal-browser` で認可コードフロー（PKCE）を行い、
  トークンの貼り付けを不要にする
- **なぜ今やらないか**: **SPA のリダイレクト URI をアプリ登録に足す必要があり、
  デプロイ窓なしに検証できない**（ADR-0044 決定9）。「書いたが動かしていない
  認証」を残さない。`scripts/setup-app-role.py` に SPA プラットフォームの
  登録を足す形になる見込み
- **出典**: `P2A-03` の設計（ADR-0044 決定9）で意図的に範囲外にした
