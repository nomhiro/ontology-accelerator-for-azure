...
**未解決**: **カーソルページングが無い**（大きな名前空間は `since` / `until` で期間を区切る）/ **IRI が `urn:` なので dereferenceable ではない**（外部ツールから「この版の詳細を引く」ことはできない）/ JSON-LD（`P2A-17`）。派生関係は `P2A-15` で、主体の種別は `P2A-16` で解決した（いずれも 2026-09-12）。
