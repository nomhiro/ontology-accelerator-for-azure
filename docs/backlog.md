...
Phase 2 で完了したもの: `P2A-05`（SHACL 検証）/ `P2A-06`（名前空間 RBAC と四眼原則）/ `P2A-09`（`platform-admin` の設定の自動化。**実機確認は未了**）/ `P2A-12`（CI が「緑なのに何も検証していない」状態を作らないようにした）/ `P2B-01`（OWL 推論器を CI へ）/ `P2B-02`（保持ポリシー）/ `P2B-03`（廃止のライフサイクル）/ `P2B-04`（用語単位の責任者）/ `P2B-05`（アクセスログ）/ `P2B-06`（健全性指標）/ `P2B-07`（想定質問の CI 実行）/ `P2B-08`（`reason` の記録と参照）/ `P2B-09`（意味的差分の計算）/ `P2B-11`（監査を読み出す API）/ `P2B-10`（領域間マッピング）/ `P2B-12`（削除と公開の競合）/ `P2B-14`（想定質問をデプロイ済みの名前空間に紐づける）。

**次に着手する候補**（費用が発生しないものを優先順に）:

| ID | 内容 | なぜ次か |
|---|---|---|
| `P2A-10` | 既存スクリプトの日本語出力を `ontology_core.console` に寄せる | `P2A-09` で仕組みは入った。**Windows で cp932 の混在ログが出る**のは残っている |
| `P2A-11` | パスパラメータ名の `{namespace}` / `{name}` の不統一 | 実害は無いが、OpenAPI から型を生成する側で名前が揺れる |
| `P2A-13` | `FUSEKI_PORT` が `scripts/check-questions.py` に効かない | ポート衝突の回避手段が CLI には届かず、**別プロジェクトの Fuseki に当たって 405 になる** |
| `P2B-15` | 推論で導出された含意をストアに射影するか決める | `P2B-01` で推論は CI に入った。**射影するかは別の決定**（ADR-0002 の「ストアは正本の射影」と「推論で増えたトリプルは正本に無い」の関係を決める必要がある） |
| `P2B-16` | 質問集合にも四眼原則を掛けるか決める | `P2B-14` の未解決。**`owner` は基準を書き換えて自分で承認できる**（ADR-0022 決定6 は「防止ではなく可視化」に留めた） |
| `P2B-18` | マッピングの先が廃止されたときに気づく | `P2B-10` の未解決。**廃止検査は名前空間ごとの TTL を見るので、PostgreSQL のマッピングが視界に入らない**。相手の名前空間の TTL を読む必要があり権限の設計も絡む |
| `P2B-17` | 領域間マッピングをストアに射影するか決める | `P2B-10` の未解決。ADR-0023 決定7 は「API と MCP で提供する」に留めた。**SPARQL だけを使うクライアントからは見えない** |
| `P2B-19` | 公開済みオントロジーを含む名前空間をどう退役させるか | `P2B-12` から分離。**409 のメッセージが「Phase 2（監査経路）で対応します」と約束している**が、不変条件7（公開済みの版は削除しない）との関係が未決（ADR-0024 決定4 に選択肢を 3 つ書いた） |
| `P2B-20` | 孤児の `_state.json` を検出する | `P2B-12` の残り。`reconcile` の `orphan_blobs` は `.ttl` だけを見るので検出できない。**害は「誰も読まないファイルが 1 つ残る」に留まる** |
...
| `P2A-13` | `FUSEKI_PORT` が `scripts/check-questions.py` に効かない | 低 | 未着手 |
...
`P2A-13` の補足（**未着手**。`P2B-14` の検証中に発見）: **ポート衝突の回避手段が CLI に届いていない。**
...
**直し方の候補**: `Settings` が `FUSEKI_PORT` を読んで既定の URL を組み立てる（3 つのエンドポイントすべてに効く）か、CLI に `--endpoint` を足す。前者の方が「テストと同じ変数で揃う」という説明が保てる。**出典**: `P2B-14` の最終検証で `just check-questions` 相当を回したときに踏んだ。
...
| `P2B-15` | 推論で導出された含意をストアに射影するか決める | 決定 1 | 低 | 未着手 |
| `P2B-16` | 質問集合にも四眼原則を掛けるか決める | 決定 6 | 低 | 未着手 |
| `P2B-17` | 領域間マッピングをストアに射影するか決める | 決定 8 | 低 | 未着手 |
| `P2B-18` | マッピングの先が廃止されたときに気づく | 決定 3・8 | 中 | 未着手 |
| `P2B-19` | 公開済みオントロジーを含む名前空間をどう退役させるか | — | 中 | 未着手 |
| `P2B-20` | 孤児の `_state.json` を検出する | — | 低 | 未着手 |
