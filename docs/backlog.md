...
Phase 2 で完了したもの: `P2A-05`（SHACL 検証）/ `P2A-06`（名前空間 RBAC と四眼原則）/ `P2A-09`（`platform-admin` の設定の自動化。**実機確認は未了**）/ `P2B-04`（用語単位の責任者）/ `P2B-07`（想定質問の CI 実行）/ `P2B-08`（`reason` / `diff` の記録と参照）。

**次に着手する候補**（費用が発生しないものを優先順に）:

| ID | 内容 | なぜ次か |
|---|---|---|
| `P2B-11` | 監査を読み出す API | `P2B-08` で `reason` を書き始めた。名前空間単位・主体単位で読める口が無いと監査に使えない。`P2B-04` で決めた責任者を MCP に出す設計もここに含める（ADR-0015 が送った） |
| `P2B-09` | 意味的差分の計算と差分レビュー | `diff` 列は `P2B-08` で用意したが常に `null`。`P2B-04` の責任者が「この差分を誰に見せるか」に答えられるようになった。承認画面（`P2A-03`）の前提 |
| `P2B-06` | 健全性指標の集計と提示 | `P2B-04` で「責任者が未設定の用語」の原資料ができた。ただし 6 項目のうち残りはアクセスログ（`P2B-05`）に依存する |
| `P2B-14` | 想定質問をデプロイ済みの名前空間に紐づける | `P2B-07` はリポジトリ内の質問を CI で回すところまで。承認をブロックするかの判断が要る |

**費用が発生するもの**（ユーザーの確認が必要）: `P2A-09` の**実機確認**。自動化（`just setup-app-role` と `preprovision` のゲート）は 2026-09-09 に完了し、ローカルでは 268 件のテストで検証済みだが、**実 Entra テナントに対しては一度も実行していない** — アプリロールの定義（Graph への PATCH）、割り当て、実トークンの `roles` クレーム、`platform-admin` を持たない主体が 403 になること、`platform-admin` が四眼原則を飛び越えられないことは、いずれもデプロイ窓が必要。
...
**見つけたが直していない**: エージェント（MCP）への露出。「この用語は誰に聞けばよいか」をエージェントが引ければ、答えの根拠を示す経路が強くなる。ADR-0015 はこれを範囲外とし、`P2B-11`（監査を読み出す API）と合わせて設計することにした。
