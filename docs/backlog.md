...
最終更新: 2026-09-12（`P2A-01` の実装後。**費用が発生しない実装はすべて完了した**）
...
**費用が発生するもの**（ユーザーの確認が必要）: `P2A-09` の**実機確認**と `P2A-18`（`idtyp: "user"` が実際に乗るかの確認。`P2A-09` の窓で同時に確かめられる）。自動化（`just setup-app-role` と `preprovision` のゲート）は 2026-09-09 に完了し、ローカルでは 416 件のテストで検証済みだが、**実 Entra テナントに対しては一度も実行していない** — アプリロールの定義（Graph への PATCH）、割り当て、実トークンの `roles` クレーム、`platform-admin` を持たない主体が 403 になること、`platform-admin` が四眼原則を飛び越えられないことは、いずれもデプロイ窓が必要。
