...
Phase 2 で完了したもの: `P2A-05`（SHACL 検証）/ `P2A-06`（名前空間 RBAC と四眼原則）/ `P2A-09`（`platform-admin` の設定の自動化。**実機確認は未了**）/ `P2B-04`（用語単位の責任者）/ `P2B-07`（想定質問の CI 実行）/ `P2B-08`（`reason` / `diff` の記録と参照）/ `P2B-11`（監査を読み出す API）。

**次に着手する候補**（費用が発生しないものを優先順に）:

| ID | 内容 | なぜ次か |
|---|---|---|
| `P2B-09` | 意味的差分の計算と差分レビュー | `diff` 列は `P2B-08` で用意したが常に `null`。`P2B-04` の責任者が「この差分を誰に見せるか」に答えられるようになった。承認画面（`P2A-03`）の前提であり、**ADR-0009 が未解決として残した「意味的差分の計算方法」を決める必要がある** |
| `P2B-05` | アクセスログの実装 | 健全性指標（`P2B-06`）の 6 項目のうち 3 項目（未参照の用語、再承認が古い、参照の傾向）がこれに依存する。ADR-0006 §4 の「エージェントに何を返したか」 |
| `P2B-06` | 健全性指標の集計と提示 | `P2B-04` で「責任者が未設定の用語」、`P2B-11` で監査の照会ができた。ただし 6 項目のうち残りは `P2B-05` に依存する |
...
**取り込まなかったもの**: `diff`（意味的差分）の計算は `P2B-09`。埋めたふりをせず `None` のまま返す。汎用の監査照会（名前空間全体・期間・実行者での絞り込み、ページング）は `P2B-11`（**2026-09-10 に完了**）。
