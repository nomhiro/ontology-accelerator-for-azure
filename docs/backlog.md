...
最終更新: 2026-09-12（`P2A-01` の実装後。**費用が発生せず、設計の好みも絡まない実装はすべて完了した** — 残りは「デプロイ窓（課金）」か「人の好み」を待っている）
...
| ID | 内容 | 何が要るか |
|---|---|---|
| `P2A-02` | LLM によるオントロジー候補生成（OWL/SHACL） | **Microsoft Foundry のモデル配備（課金）。** プロンプトと検証ループが価値の中心であり、フェイクに対して緑になっても何も言えない |
