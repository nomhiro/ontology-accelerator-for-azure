...
Phase 2 で完了したもの: `P2A-05`（SHACL 検証）/ `P2A-06`（名前空間 RBAC と四眼原則）/ `P2B-07`（想定質問の CI 実行）/ `P2B-08`（`reason` / `diff` の記録と参照）。

**次に着手する候補**（費用が発生しないものを優先順に）:

| ID | 内容 | なぜ次か |
|---|---|---|
| `P2B-04` | 名前空間と用語の責任者 | `P2A-06` で権限の器ができた。責任者は「誰が答えるべきか」の層で、RBAC とは別物。`P2B-13` の残りと健全性指標（`P2B-06`）の前提 |
| `P2B-11` | 監査を読み出す API | `P2B-08` で `reason` を書き始めた。名前空間単位・主体単位で読める口が無いと監査に使えない |
| `P2B-09` | 意味的差分の計算と差分レビュー | `diff` 列は `P2B-08` で用意したが常に `null`。承認画面（`P2A-03`）の前提 |
| `P2B-14` | 想定質問をデプロイ済みの名前空間に紐づける | `P2B-07` はリポジトリ内の質問を CI で回すところまで。承認をブロックするかの判断が要る |

**費用が発生するもの**（ユーザーの確認が必要）: `P2A-09`（`platform-admin` アプリロールの設定の自動化と実機確認）。**これは `P2A-06` の副作用として新しく発生した前提である** — 名前空間の作成に `platform-admin` が必要になったため、割り当てが無いと `azd up` の `postdeploy` が 403 で止まる。README に手順を書き `postdeploy` が原因を言い切るようにしたが、設定は手作業のままで実機確認も未実施。
...
- **Phase 2 でやること**: `P1-16` の最小実装に**権限の強制**を足す
  ...
  - **残り**: 責任者のみが `approve` できる（`P2B-04` に依存）。RBAC の `maintainer` は「承認してよい人」であり、`P2B-04` の責任者は「その用語について答えるべき人」で、層が違う
...
| `P2A-09` | `platform-admin` アプリロールの設定を自動化する | 高 | 未着手 |
...
`P2A-09` の補足（**未着手**。`P2A-06` で発見）: `platform-admin` アプリロールの定義と割り当てが手作業である。

- **なぜ問題か**: `P2A-06` 以降、**名前空間の作成には `platform-admin` が必要**である。`azd up` の `postdeploy` は同梱サンプルの名前空間を作るため、**割り当てが無いと `azd up` が 403 で止まる**。Phase 1 の完了条件（`azd up` 一発で SPARQL が答える）に直接かかる
- **なぜ Bicep で作れないか**: Entra のアプリ登録は ARM のリソースではなく、`azd` の管理外にある（`azd down` でも消えない。`P1-09` に記録）。Bicep からアプリロールを定義することはできない
- **やること**:
  1. `az` でアプリロールを定義・割り当てるスクリプト（`scripts/setup-app-role.sh` 相当）を用意し、README から手順を移す。**`appRoles` は複合プロパティなので部分更新すると既存が消える**（`api` で同じ罠を踏んでいる。`P1-09` の罠1）
  2. `preprovision` で割り当ての有無を確認し、無ければ**デプロイ前に**止める（`postdeploy` の 403 まで進むと約 11 分と課金を無駄にする）
  3. **実機で確認する**: 実トークンに `roles` クレームが乗ること、`platform-admin` を持たない主体が名前空間の作成で 403 になること、`platform-admin` が四眼原則を飛び越えられないこと（ADR-0014 決定5）
- **費用**: 実機デプロイ 1 回。**ユーザーの確認が必要**
- **出典**: `P2A-06` の実装（2026-09-09）。README に手順を書き `postdeploy` に 403 のヒントを入れたが、自動化と実機確認は未了
...
**取り込まなかったもの**: `diff`（意味的差分）の計算は `P2B-09`。埋めたふりをせず `None` のまま返す。汎用の監査照会（名前空間全体・期間・実行者での絞り込み、ページング）は `P2B-11`。
