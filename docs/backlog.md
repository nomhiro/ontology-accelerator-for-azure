...
現在は Phase 3 を進めている（`P3-01` が完了。**実データを実体化せずに
SPARQL で引ける**ようになった）。
...
Phase 3 で完了したもの: `P3-01`（Ontop VKG。**宛先を間違えると 0 件が返ることを実測したので、宛先を URL で分けた**）/ `P3-05`（定義と実データの乖離検出。**pyshacl を実データに当てず、形から SPARQL の探りを組み立てる**）/ `P3-08`（仮想グラフのアクセスログ。**`NULL` = オントロジーへの照会にして、既存の行の意味を変えなかった**）。

**下の表に残っているのは `P2A-21` と `P3-07` の 2 件だけである**（Phase 3 の残り `P3-02`〜`P3-04` / `P3-06` / `P3-09` と Phase 4 は別表にある）。どちらも**ローカルで検証できない**（デプロイ窓が必要）。判断だけで進めると「緑なのに何も検証していない」状態を作る。
