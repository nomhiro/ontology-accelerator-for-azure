...
SPARQL で引ける**ようになり、**用語を自然文で探せる**ようになった）。

Phase 2 で完了したもの: `P2A-05`（SHACL 検証）/ `P2A-06`（名前空間 RBAC と四眼原則）/ `P2A-09`（`platform-admin` の設定の自動化。**2026-09-13 に実機確認済み**）/ `P2A-12`（CI が「緑なのに何も検証していない」状態を作らないようにした）/ `P2B-01`（OWL 推論器を CI へ）/ `P2B-02`（保持ポリシー）/ `P2B-03`（廃止のライフサイクル）/ `P2B-04`（用語単位の責任者）/ `P2B-05`（アクセスログ）/ `P2B-06`（健全性指標）/ `P2B-07`（想定質問の CI 実行）/ `P2B-08`（`reason` の記録と参照）/ `P2B-09`（意味的差分の計算）/ `P2B-11`（監査を読み出す API）/ `P2B-10`（領域間マッピング）/ `P2A-10`（スクリプトの cp932 出力）/ `P2A-08`（結果件数の上限の強制）/ `P2A-11`（パスパラメータ名の統一）/ `P2A-13`（`FUSEKI_PORT` が CLI に届かない）/ `P2B-12`（削除と公開の競合）/ `P2B-14`（想定質問をデプロイ済みの名前空間に紐づける）/ `P2A-07`（監査証跡の PROV-O 表現）/ `P2A-15`（何から編集したかの記録）/ `P2B-15`（導出された含意を射影するかの決定 → **射影しない**）/ `P2B-16`（受け入れ基準の出自の検査）/ `P2B-18`（マッピング先の生死）/ `P2B-17`（マッピングを射影するかの決定 → **射影せず Turtle で書き出す**）/ `P2B-19`（名前空間の退役）/ `P2B-20`（孤児のマニフェスト）/ `P2A-14`（`CONSTRUCT` / `DESCRIBE`）/ `P2A-16`（主体の種別の記録）/ `P2A-17`（監査証跡の JSON-LD 表現）/ `P2B-21`（廃止された先を指すマッピングを指標に出す）/ `P2B-23`（RDF の結果への廃止警告）/ `P2B-22`（束縛が無い結果の行数）/ `P2B-13`（承認 API と状態遷移。**最後の判断は「責任者を承認の条件にしない」**）/ `P2A-01`（顧客 DB のスキーマ自動発見。**実データを 1 行も読まない**）/ `P2A-19`（スキャンの定期実行。**既定で動かない**）/ `P2A-02`（LLM によるオントロジー候補生成。**候補は `draft` にしか入らない**）/ `P2A-03` / `P2A-04`（Web のレビュー・承認とグラフ可視化。**「計算できなかった」を「変更が無い」と同じ見た目にしない**）/ `P2A-20`（Web の MSAL 認証。**後回しにした理由が誤っていた**）。

**柱 B は全件完了した（2026-09-12）。柱 A も、費用が発生しない実装はすべて完了した（2026-09-12、`P2A-01`）。**

Phase 3 で完了したもの: `P3-01`（Ontop VKG。**宛先を間違えると 0 件が返ることを実測したので、宛先を URL で分けた**）/ `P3-05`（定義と実データの乖離検出。**pyshacl を実データに当てず、形から SPARQL の探りを組み立てる**）/ `P3-08`（仮想グラフのアクセスログ。**`NULL` = オントロジーへの照会にして、既存の行の意味を変えなかった**）/ `P3-02`（用語のベクトル検索。**Azure AI Search を採らず、正本の PostgreSQL に載せて追加費用 $0 にした**）。

**下の表に残っているのは `P2A-21` と `P3-07`、そして `P3-02` の実機確認の 3 件である**（Phase 3 の残り `P3-03` / `P3-04` / `P3-06` / `P3-09`〜`P3-11` と Phase 4 は別表にある）。**どれもローカルで検証できない**（デプロイ窓が必要）。判断だけで進めると「緑なのに何も検証していない」状態を作る。

| ID | 内容 | 何が要るか |
|---|---|---|
| ~~`P2A-02`~~ | ~~LLM によるオントロジー候補生成（OWL/SHACL）~~ | **完了（2026-09-13 に実機確認）。実モデルが 1 回目で検証を通った** |
| ~~`P2A-19`~~ | ~~スキャンの定期実行（ACA Job）~~ | **完了（2026-09-13 に実機確認）。ジョブが 33 秒で `Succeeded`** |
| ~~`P2A-09`~~ | ~~`platform-admin` アプリロールの**実機確認**~~ | **完了（2026-09-13）。ただし 4 項目のうち 1 つ（403 になるか）は別の主体が必要で未確認** |
| ~~`P2A-18`~~ | ~~`idtyp: "user"` が実トークンに乗るかの確認~~ | **完了（2026-09-13）。乗る** — [ADR-0035 補記1](adr/0035-actor-type.md) |
| ~~`P2A-03`~~ | ~~Web でのレビュー・承認フロー~~ | **完了（2026-09-13、[ADR-0044](adr/0044-review-ui.md)）** |
| ~~`P2A-04`~~ | ~~グラフ可視化~~ | **完了（2026-09-13）** |
| ~~`P2A-20`~~ | ~~Web の認証を MSAL にする~~ | **完了（2026-09-13、[ADR-0045](adr/0045-web-auth.md)）。後回しにした理由が誤っていた** — localhost の登録にデプロイ窓は要らなかった |
| `P2A-21` | デプロイ済みオリジンでの Web のサインイン | **デプロイ窓（課金）。** Static Web Apps のホスト名が provision 後にしか分からない |
...
**`P2A-01` で「メタデータだけを読む」を構造で固定した。** 発行する SQL は `ontology_core.scan.CATALOG_QUERIES` の 5 本に限られ、**テストが「`pg_catalog` / `information_schema` 以外を参照していないこと」を機械的に検査する**（[ADR-0041](adr/0041-source-schema-scan.md) 決定3）。`P2A-02` がこのカタログを LLM のプロンプトへ流すので、**実データが混ざったら消せない**のがその理由である。

**デプロイ窓は 2026-09-13 に開けて閉じた。9 項目すべて確認した**
（`azd up` 14 分 46 秒 / 検証 約 7 分 / `azd down --purge` 26 分 56 秒。
**撤収は副作用で確認した** — `az group exists` が false、Key Vault と
Cognitive Services の論理削除も残っていない）。結果は
[デプロイ窓の実機確認](#デプロイ窓の実機確認完了2026-09-13)にある。

**窓で確かめられなかったことは 1 つある。** `platform-admin` を持たない主体が
403 になることは、**別の主体を用意しない限り確かめられない**（手元のトークンには
ロールが付いている）。ローカルのテストでは確認済みだが、**実機では「確かめた」と
書かない**。

**次に窓が必要なのは `P2A-21` と `P3-07` である。** 前者は Static Web Apps の
ホスト名が provision 後にしか分からないため、後者は Ontop を ACA に載せる
ためである。**`P3-07` は設計が先である** — 「1 ソース = 1 インスタンス」を
Bicep でどう表現するかが決まっていない。確かめられることを下にまとめておく
（窓は高いので 1 回で取りきる）。

| 何を | どのタスク |
|---|---|
| デプロイ済みオリジンを `spa` のリダイレクト URI として登録して、サインインできるか | `P2A-21` |
| `azd deploy` が `VITE_ENTRA_CLIENT_ID` をビルド時に渡すか（**未確認**） | `P2A-21` |
| `platform-admin` を持たない主体が 403 になるか（要: 2 つ目の主体） | `P2A-09` |
| Ontop が内部 ingress で立ち、マッピングを API から取って起動するか | `P3-07` |
| **Entra トークン認証のソースで仮想グラフを立てられるか**（**未検証**） | `P3-07` |
...
- **Phase 2 でやること**: `P1-16` の最小実装に**権限の強制**を足す
  ...
  - ~~**残り**: 責任者のみが `approve` できる~~ → **`ADR-0040` で「条件にしない」と決めた**（2026-09-12）。**どちらの分岐も悪い**ためである — 責任者がいない用語を含む版を承認できなくすれば**人質**（ADR-0030 決定7 が却下した形）、責任者がいなければ誰でも承認できるとすれば**不変条件11 が最も避けたい形**になる。「全用語の責任者」は版単位の承認（ADR-0010 決定2）と噛み合わず（50 用語なら50 人）、「1 人でよい」は `maintainer` 以上を既に要求しているので**制約が増えていない**。さらに、責任者は `maintainer` が付け替えられる（ADR-0015 決定5）ので、条件にすると**責任者の付与が承認権限の付与になる**（不変条件13 が「混ぜるな」と言っているのはこの形）。代わりに**差分レビューで問い合わせ先を返す**（ADR-0015 が「得られるもの」に挙げていて未実装だった約束）
...
| ID | 内容 | 優先 | 状態 |
|---|---|---|---|
| `P2A-01` | 顧客 DB 接続とスキーマ自動発見（scan-job） | 高 | **完了**（2026-09-12） |
| `P2A-02` | LLM によるオントロジー候補生成（OWL/SHACL） | 高 | **完了**（2026-09-12 に実装、**2026-09-13 に実機確認**。実モデルが 1 回目で検証を通った） |
| `P2A-03` | Web でのレビュー・承認フロー | 高 | **完了**（2026-09-13） |
| `P2A-04` | グラフ可視化（Cytoscape.js 等） | 中 | **完了**（2026-09-13） |
| `P2A-20` | Web の認証を MSAL にする | 中 | **完了**（2026-09-13） |
| `P2A-21` | デプロイ済みオリジンでの Web のサインイン | 中 | 未着手（**要デプロイ窓**） |
| `P2A-22` | ロックファイルが 2 つある（npm と pnpm） | 低 | **完了**（2026-09-13） |
...
| `P2A-16` | 主体が人間かサービスプリンシパルかを記録する | 低 | **完了**（2026-09-12） |
| `P2A-18` | `idtyp: "user"` が v2.0 のアクセストークンに乗るかの実機確認 | 低 | **完了**（2026-09-13。**乗る**） |
| `P2A-17` | 監査証跡の JSON-LD 表現 | 低 | **完了**（2026-09-12） |
| `P2A-09` | `platform-admin` アプリロールの設定を自動化する | 高 | **完了**（2026-09-09 に自動化、**2026-09-13 に実機確認**） |
...
| `P2A-19` | スキャンの定期実行（ACA Job の Bicep） | 中 | **完了**（2026-09-12 に実装、**2026-09-13 に実機確認**） |

### `P2A-01` 顧客 DB のスキーマ自動発見（**完了。2026-09-12**）

設計は [ADR-0041](adr/0041-source-schema-scan.md)（決定 10 件）。柱 A の入口であり、**カタログは `P2A-02` の入力になる**。

**設計の中心は「実データを 1 行も読まない」である**（決定1）。カタログは LLM のプロンプトへ流れるので、**一度混ざった実データは、後から消してもモデルの提供者へ渡った事実が消えない**。そこで

- 発行する SQL を `ontology_core.scan.CATALOG_QUERIES` の 5 本に固定し（決定3）
- **テストが「`pg_catalog` / `information_schema` 以外を参照していないこと」と「`most_common` / `histogram` が現れないこと」を機械的に検査する**
- さらに **API のテストが、実データを入れて `ANALYZE` したテーブルの値がカタログに現れないことを実物で確かめる**（`pg_stats.most_common_vals` には**列の値そのもの**が入るため）

**「無い」と「0」を区別する**（決定2）。`reltuples = -1` または `ANALYZE` の記録が無いときは `estimated_rows = None`、`reltuples = 0` かつ記録ありのときだけ `0` を書く。`n_distinct` の**負の値は行数に対する比率**なので、絶対数に換算せず別の列（`distinct_ratio`）に入れる — 換算には行数の推定が必要で、それが `None` のときに**存在しない数を作る**ことになる。

**資格情報を受け取らない**（決定4）。登録の本文に秘密の欄が無く、`scan_sources` に値を持つ列が無い。`key-vault-secret` のときは**名前だけ**を持ち、値は接続のたびに Key Vault から解決する。**Key Vault の応答本文は例外にもログにも載せない**（秘密が入りうる）。

**接続先はホストの allowlist で絞り、既定は空である**（決定5）。任意のホストへ接続できる口は、認証済みの主体に**内部ネットワークの到達性を調べる手段**を与える（SPARQL の `SERVICE` を既定で禁止したのと同じ判断）。**登録の時点で 403 にする** — 登録できてしまうと「後で接続できるはず」という誤解が残る。

**途中で落ちたスキャンを完了した観測と混ぜない**（決定7）。`scan_runs` に `running` を先に書いて commit し、失敗したら `failed` にする。**`table_count` は `None` のままにする** — 「0 件だった」と「まだ分からない」を混ぜない。カタログの既定は**最後に `succeeded` した run** で、`run_id` を明示すれば失敗した観測も読める（どこまで読めたかは診断の材料である）。

**実物の PostgreSQL に対して回して初めて分かったことが 2 つある。**

1. **`pg_class.relkind` と `pg_constraint.contype` は asyncpg から `bytes` で返る**（`"char"` は 1 バイトの内部型）。`str(b"r")` は `"b'r'"` になるので、テーブルの種別が対応表から静かに外れ、**主キーと外部キーが「1 件も無い」ことになっていた**。後者が悪い — **「読めなかった」が「無かった」として通る**。SQL 側で `::text` に寄せ、読み手でも `bytes` を受けられるようにした（回帰テストあり）
2. **`create_async_engine(..., poolclass=None)` はプールを無効にしない**（`None` は「既定のプールを使う」の意味）。`NullPool` を明示していないのに「プールしない」とコメントしていた。**顧客 DB への接続がプロセスに残り続ける**

どちらも CLAUDE.md の「既知の罠」に記録した。**1 は「フェイクでは再現しない」種類の不具合である** — この機能のテストの中心を「自分自身の DB を実際にスキャンする」に置いたことで見つかった。

**カタログは MCP に出さない**（ADR-0041 の却下案）。カタログは*観測*であって、レビューも承認も受けていない。エージェントに出すのは `approved` の版だけである（ADR-0010）ため、出すと**審査を通っていない語彙がエージェントの文脈に入る経路**ができる。表と列の名前それ自体が機微であることも理由である。**「まだ出していないだけ」に見えないよう、`packages/mcp-server/tests/test_tool_inventory.py` が出しているツールを数え上げて固定した** — ツールを足すと落ちるので、出してよい理由を確かめる機会が必ず来る。

**ACA Job はこの ADR では作らない**（決定10）。Bicep は `az bicep build` で構文までしか確かめられず、**課金なしに動作を検証できない**。「書いたが動かしていない IaC」を残すのはこのリポジトリが避けている形なので、`P2A-19` として記録した。

### `P2A-20` Web の認証を MSAL にする（**完了。2026-09-13**）

設計は [ADR-0045](adr/0045-web-auth.md)（決定 10 件）。

**着手して分かったのは、後回しにした理由が誤っていたことである。**
ADR-0044 決定9 はこう書いていた。

> **MSAL を入れない。** SPA のリダイレクト URI はアプリ登録の変更を伴い、
> **デプロイ窓なしに検証できない**。

**「登録」と「デプロイ済みオリジンの検証」を混同していた。**

| 何を | デプロイ窓が必要か |
|---|---|
| `http://localhost:5173` を SPA のリダイレクト URI として登録する | **不要**（Graph への PATCH だけ。課金なし） |
| デプロイ済みオリジンを登録する | 必要（ホスト名が provision 後にしか分からない） |
| 対話的なサインインの往復を確かめる | **人がブラウザで認証する必要がある**（デプロイ窓とは別） |

1 番目が不要だったので、**貼り付けをやめて MSAL を入れた**。
ADR-0044 に補記1 を書いて撤回を記録した。

**Entra の文書から測った 3 つの事実**（ADR-0045 決定3・4）:

| 事実 | 帰結 |
|---|---|
| **HTTP は localhost だけ例外** | デプロイ環境では https を強制する |
| **localhost ではポートが照合で無視される** | 1 つ登録すれば `vite preview`（4173）でも通る。**ポートだけ違う URI を足してはいけない**（サーバが任意に 1 つ選ぶ） |
| **`spa` 型で登録しないと CORS で失敗する** | `web` 型では動かない |

`scripts/setup-app-role.py` が `spa.redirectUris` を登録する。
**複合プロパティの罠**（`appRoles` / `optionalClaims` と同じ。部分更新すると
既存が丸ごと消える。`P1-09` の罠1）を `merge_spa_redirect_uri` で受け止め、
**localhost を 2 つ登録しようとしたら拒否する**。

**ほかの決定**:

- **リダイレクト方式。ポップアップを使わない**（決定2）。ポップアップの
  ブロックは**静かな失敗**である
- **トークンは `sessionStorage`**（決定7）。ADR-0044 の「メモリのみ」は
  **撤回した** — リダイレクトはページ遷移をまたぐので、メモリだけでは
  PKCE の verifier が消えてフローが成立しない。`localStorage` は却下
  （XSS で持ち出されたトークンが再起動後も有効になる）
- **貼り付けの口を削除した**（決定8）。2 つの経路を残すと、MSAL が動かない
  ときに貼り付けへ逃げる運用が生まれ、**「サインインが壊れている」ことに
  誰も気づかない**
- **設定が無ければ MSAL を初期化しない**（決定9）。半端に初期化すると、
  押したときに `AADSTS900971` のような**設定の誤りに見えないエラー**になる
- **クライアント用のアプリ登録を分けない**（決定5）。テナントへの書き込みが
  増える割に、得るものが少ない

**実測で分かったこと**:

- **`.default` と `user_impersonation` は同じトークンを生む。**
  `aud=<appId>` / `scp=user_impersonation` / `roles=[platform-admin]` /
  `idtyp=user` で、**デプロイ窓で API が受け入れたトークンと同じ形**である。
  つまり決定6 は読みやすさの選択であって、動作の違いではない
- **MSAL の型に自前の型を合わせる必要があった。** `MsalLike` を狭く作ったら
  実物の `PublicClientApplication` を渡せなかった。**キャストで潰さず実物の
  形に合わせた** — 潰すと偽物だけが通るテストになる

**確かめていないこと**:

- **対話的なサインインの往復は検証していない。** 資格情報の入力を伴うため、
  こちらでは行わない。README に手順を書いたので、**運用者が 1 回試す**
- **デプロイ済みオリジンでの動作は `P2A-21`**

Web のテスト 58 → **89 件**。`@azure/msal-browser`（MIT、約 256 kB）は
**認証を設定したときだけ動的に読み込む**。

---

### `P2A-22` ロックファイルが 2 つあった（**完了。2026-09-13**）

- **状態**: **完了**（2026-09-13）。`apps/web/package-lock.json` を削除し、
  `.gitignore` に `package-lock.json` / `npm-shrinkwrap.json` を追加した
- **由来が分かったので消した。** 未着手のときの懸念は「`.gitignore` にも
  入っていないので、意図して置かれた可能性がある」だった。
  `git log --follow` で追うと**コミットは 1 つだけ**で、それは
  `da37b7e`「fix(mcp): Container Apps の FQDN で 421 Invalid Host header に
  なるのを修正」だった。**そのコミットが触った他のファイルは
  `infra/` と `packages/{core,mcp-server}` だけ** — つまり
  **MCP の修正中に `npm install` を実行して、その副産物が一緒に
  コミットされた**ものであり、意図的な追加ではなかった
- **消してよいと判断した根拠**:
  - **由来が事故である**（上記）
  - **リポジトリ内のどこからも参照されていない**（`grep` で確認。
    CI も `justfile` も pnpm しか使わない）
  - **実測で `npm ci` が既に失敗する**
    （`Missing: stackback@0.0.2 from lock file`）。2026-08-28 のまま
    古くなっており、その後に足した cytoscape と `@azure/msal-browser` の
    解決情報が無い
- **`.gitignore` に入れたのは再発を防ぐため。** 事故の形（`npm install` を
  叩いて `git add -A`）に対して効く唯一の構造的な手当てである
- **確かめていないこと**: `azure.yaml` は `apps/web` を `language: js` で
  ビルドするので、**azd がどちらのパッケージマネージャを選ぶかは実機で
  確かめていない**。壊れたロックファイルがあると npm を選んで失敗しうるが、
  **消した今はその曖昧さ自体が無い**
- **出典**: `P2A-03` の実装中に `pnpm install` の前提を確認していて気づいた
  （2026-09-13）

---

### `P2A-21` デプロイ済みオリジンでの Web のサインイン

- **状態**: 未着手（**要デプロイ窓**）
- **優先**: 中
- **内容**: Static Web Apps のオリジンを SPA のリダイレクト URI として登録し、
  デプロイ済みの画面からサインインできることを確かめる

```bash
# provision 後にホスト名が分かる
azd env get-values | grep SERVICE_WEB_URI
uv run python scripts/setup-app-role.py --spa-redirect-uri "https://<host>"
```

- **もう 1 つ未確認のこと**: **`VITE_*` はビルド時にバンドルへ埋め込まれる。**
  azd の `deploy` が web をビルドするとき、`azd env` の値が
  `VITE_ENTRA_CLIENT_ID` として渡るかを確かめていない。渡らなければ
  `azure.yaml` か `infra/modules/web.bicep` に手当てが必要になる
- **なぜ今やらないか**: ホスト名が provision 後にしか分からず、
  **デプロイ窓が必要**（`azd up` 約 11 分 + `azd down --purge` 約 24 分）
- **出典**: `P2A-20` の設計（ADR-0045 決定10）で意図的に範囲外にした

---

### `P2A-03` / `P2A-04` Web でのレビュー・承認とグラフ可視化（**完了。2026-09-13**）

設計は [ADR-0044](adr/0044-review-ui.md)（決定 9 件）。

**「見た目と操作の設計（人の好み）」として後回しにしていたが、半分は好みでは
なかった。** 配色や余白は好みである。しかし**この画面は人が「承認する」と決める
場所**であり、API が既に知っている事実を画面が落とすと、**落としたことは画面に
出ない**。

**設計の中心は決定1 である。** [ADR-0016](adr/0016-semantic-diff.md) 決定5 は
空白ノードが多すぎたら差分を**計算せず**「計算できなかった」と返すと決めた。
画面が「追加 0 / 削除 0」と出すと、**レビュアは「変更が無い」と読む**。
実際には「測っていない」で、**承認の判断が逆になる**。

だから差分を **4 つの状態**として扱う（実装して分かったが 3 つではなかった）。

| 状態 | 画面 |
|---|---|
| 基準の版が無い（最初の版） | 「比較する基準がない」と明記 |
| **計算していない** | **警告色 + 理由 + 「変更が無いという意味ではありません」** |
| 計算して変更なし | 「変更はありません」と明記 |
| 計算して変更あり | 中身を出す。**`modified_terms` が `null` のときは「測っていません」**（0 件と書かない） |

**ほかの決定**:

- **承認できない理由を文章で出す**（決定3）。無効なボタンだけだとレビュアは
  壊れていると思う。**主体が分からないときは「できる」と言い切らない** —
  押すと 422 になることがある
- **消えた用語を最も目立たせる**（決定4）。`approve` が 422 で止める条件なので、
  押した後より押す前に分かるほうがよい
- **グラフは SPARQL で引く**（決定5）。Turtle をブラウザで解析しない —
  **この製品がエージェントに提供している口をそのまま使う**（画面が動くことが
  その口の検証になる）。**`draft` は描けない**ので、空のグラフではなく
  「射影されていないため描けない = エージェントからも見えない」と出す
- **上限を超えたら「全部は描いていない」と言う**（決定5）。グラフには封筒が
  あるので切り詰めてよいが、黙ってはいけない
- **判断は純粋関数に置く**（決定6）。描画は検証できないが判断は検証できる。
  `vitest` を入れて **58 件**
- **LLM 生成であることを最上部に出す**（決定7）。試行回数も出し、
  **3 回以上なら「モデルがこのスキーマを扱いかねている」と書く**
- **`reason` を画面では必須にする**（決定8）。API では任意（`postdeploy` を
  壊さないため）だが、`P2B-08` が「人が操作する経路では必ず書かせる」と
  約束していた分である

**ADR-0004 が求めていた仕組みを作った**（決定2）。`openapi.json` と
`apps/web/src/api/schema.ts` は gitignore された生成物なので、**Web が
import した瞬間に CI の型検査が落ちる**状態だった。ADR-0004 は受け入れる
コストとして「生成漏れによる不整合を CI で検出する仕組みが必要」と書いて
いたが、**作られていなかった**。

- web ジョブに `uv` と生成ステップを足した
- **`web` の paths-filter に `packages/api/**` と `packages/core/**` を足した** —
  API を変えて Web の型が壊れても web ジョブが動かなければ、検出の仕組みとして
  意味がない

**`GET .../diff` だけは生成された型が無い**（`dict[str, Any]` を返すため
OpenAPI に `additionalProperties: true` としか出ない）。Web は手書きの型を
持つしかない（ADR-0004 の例外）。**代わりに
`packages/api/tests/test_web_contract.py` が TS のファイルを実際に読んで鍵を
突き合わせている** — TS に欄を足すと落ちることを実測で確認した。

**確かめていないこと**:

- **描画そのものを検証していない。** ブラウザを持たないので、確かめたのは
  純粋関数（58 件）・型検査・ビルドまでである。**「動作を確認した」と
  書いていない**
- **デプロイ環境での認証は `P2A-20`。** MSAL を入れていない — SPA の
  リダイレクト URI はアプリ登録の変更を伴い、**デプロイ窓なしに検証できない**
  （ADR-0042 決定5 で azd の `containerapp-job` 対応を「確認できないから
  依存しない」とした判断と同じ）。当面はトークンを貼る（**メモリのみ。
  `localStorage` に書かない**）

**バンドルを分割した。** cytoscape が約 445 kB あり、レビューの経路（主たる
用途）がその分を払う理由が無いので、グラフのタブを開いたときだけ読み込む
（`React.lazy`）。

---

## デプロイ窓の実機確認（**完了。2026-09-13**）

**1 回の窓で 9 項目すべて確認した。** `azd up` 14 分 46 秒 / 検証 約 7 分 /
`azd down --purge` 26 分 56 秒。**撤収は副作用で確認した**（`az group exists` が
`false`、Key Vault と Cognitive Services の論理削除も残っていない）。

| # | 確認したこと | タスク | 結果 |
|---|---|---|---|
| 1 | アプリロールの定義（Graph への PATCH） | `P2A-09` | **通った** |
| 2 | ロールの割り当て | `P2A-09` | **通った** |
| 3 | 実トークンの `roles` に `platform-admin` | `P2A-09` | **乗った** |
| 4 | デプロイ済み API で `platform-admin` が実際に効く | `P2A-09` | **効いた**（`postdeploy` が名前空間を作成 → `approved` まで通過） |
| 5 | `idtyp: "user"` が v2.0 のアクセストークンに乗るか | `P2A-18` | **乗った**（[ADR-0035 補記1](adr/0035-actor-type.md)） |
| 6 | ACA Job が起動して `Succeeded` で終わるか | `P2A-19` | **成功**（33 秒） |
| 7 | `postdeploy` がジョブのイメージを差し替えるか | `P2A-19` | **差し替わった**（プレースホルダではなく API のイメージ） |
| 8 | マネージド ID で PostgreSQL とソース DB に届くか | `P2A-19` / `P2A-01` | **届いた**（14 テーブル） |
| 9 | **実モデルが検証を通る Turtle を返すか** | `P2A-02` | **1 回目で通った**（`attempts: 1`） |

### 設計の判断が実機で成立したこと

**`SCAN_JOB_TRIGGER_TYPE="Manual"`**（[ADR-0042](adr/0042-scan-job.md) 決定1）。
デプロイしても**自動では 1 度も走らない**状態が、出力とジョブのリソース定義の
両方で確認できた。

**`MODEL_ENDPOINT="https://oai-<token>.openai.azure.com/"`**（ADR-0043 決定8）。
`kind: 'AIServices'` ではなく `kind: 'OpenAI'` を「1 回の窓で確かめきるために
確実さを取る」として選んだ判断が当たり、`{endpoint}/openai/deployments/...` が
そのまま成立した。

**候補は `draft` のまま射影されなかった**（ADR-0043 決定5）。
`status=draft` / **`projected_at=None`** = エージェントから見えない。

**`scan_runs` が人間と機械に分かれた**（ADR-0042 決定2）。
API から実行した run は `started_by` が私の `oid`、ジョブが実行した run は
`AZURE_CLIENT_ID`（マネージド ID）だった。

**監査に出自が残った**（ADR-0043 決定10）:
...
### 生成されたオントロジーの中身（`P2A-02` の質）

**検証を通るだけのゴミではなかった。** 自分自身の DB の 3 テーブル
（`namespaces` / `ontology_versions` / `term_owners`）から 356 行。

| 何が出たか | 数 |
|---|---|
| `owl:Class` | 3（テーブルごとに 1 つ） |
| `owl:DatatypeProperty` | 24 |
| **`owl:ObjectProperty`** | **1**（外部キー `ontology_versions.namespace → namespaces.name` を関係にした） |
| `sh:NodeShape` | 3（`NOT NULL` を `sh:minCount 1`、型を `sh:datatype` に） |
| `base_iri` の外の主語 | **0** |

**外部キーを関係に変えたことが、スキーマからオントロジーへの帰納が
起きている証拠である。** 列を機械的にプロパティへ写しただけなら
`owl:ObjectProperty` は出ない。
...
| 何が起きたか | どこ |
|---|---|
| `subprocess(..., text=True, encoding="utf-8")` が `az` の cp932 警告で落ち、`proc.stdout` が `None` になって **`AttributeError` という原因を指さない例外**になった | `scripts/setup-app-role.py` |
| `--body '{...}'` が **Windows の `az.cmd` に再解析されて壊れ**、`"..." の使い方が誤っています` という az でも Graph でもないエラーになった | 同上 |
| **`--dry-run` が `az ad sp create` を実行していた**（実際にサービスプリンシパルが作られた） | 同上 |
| `MSYS_NO_PATHCONV=1` が `az storage blob download --file` の POSIX パスも変換しなくして落ちた | 検証手順（CLAUDE.md の既知の罠そのもの） |

### 残った未確認

**`platform-admin` を持たない主体が 403 になること**は確かめていない。
手元のトークンにはロールが付いているので、**別の主体を用意しない限り
実機では確かめられない**。ローカルのテストでは確認済みである
（`packages/api/tests/` の RBAC のテスト）。**「確かめた」と書かないために
ここに残す。**

---

### `P2A-18` `idtyp: "user"` が実トークンに乗るかの実機確認（**完了。2026-09-13**）

- **状態**: **完了。乗る**
- **優先**: 低
- **結果**: [ADR-0035 補記1](adr/0035-actor-type.md)
- **出典**: `P2A-16`（ADR-0035 決定7）が「実機で確かめていない」として残した

デプロイ窓（`P2A-09` / `P2A-19` / `P2A-02` と同時）で実測した。

```
{"idtyp": "user", "roles": ["platform-admin"], "ver": "2.0", "appid": null}
```

**`scripts/setup-app-role.py` が設定する組み合わせ**（`optionalClaims.accessToken`
に `idtyp`、`additionalProperties` に `include_user_token`）で、
**v2.0 のアクセストークンにも `idtyp: "user"` が出る**。

| これまで | 実測後 |
|---|---|
| 人間が `unknown` で記録される**かもしれない** | **`user` で記録される** |
| `prov:Person` が出ない**かもしれない** | **出る** |
| 文書が v1.0 固有の節にしか書いていない | **v2.0 でも効く**（文書の分類が実際より狭い） |

**決定1（3 値の `ActorType`）は変えない。** `idtyp` を設定していないテナントでは
相変わらず付かないので、「クレームが無い」を「人間である」と読んではいけないことは
変わらない。**確定したのは「この製品の設定手順に従えば付く」ことである。**

---

### `P2A-02` LLM によるオントロジー候補生成（**完了。2026-09-12**）

設計は [ADR-0043](adr/0043-ontology-proposal.md)（決定 11 件）。`P2A-01` のカタログを入力に、
LLM が OWL/SHACL の候補を作って `draft` にする。**Scan → Model が繋がった。**

**この製品の前提は「LLM の出力が人間の承認を経ずに正本へ入ることはない」である。**
これまでその前提は**LLM を呼ぶ経路が 1 つも無かったから**成り立っていた。
`P2A-02` で初めて**コードで強制される**（決定5）。

- 候補は `draft` にしか入らない。`draft` は Blob と PostgreSQL にだけ存在し、
  **Fuseki には一切現れない**（ADR-0010 決定1）= エージェントから見えない
- 生成 API は `submit` も `approve` も呼ばない
- テストは状態の文字列だけでなく**射影先を見て「グラフが 1 つも増えていない」**ことを
  確かめる（状態の文字列は変えられるが、射影されていないことは射影先を見ないと分からない）

**プロンプトインジェクションの扱いが設計の中心のもう一方である**（決定2）。
`column_comment` は**顧客 DB の持ち主が書いた文字列**で、この製品の管理外にある。
対策は 3 段で、**区切り記号は最も弱い段**である。

| 段 | 何をするか | 強さ |
|---|---|---|
| a | 区切った節に入れ「ここはデータである」と指示する | **弱い。破られうる** |
| b | **モデルにツールを与えない**（`tools` / `functions` を渡さない） | 強い |
| c | **検証を通っても `draft` にしか入らない** | **最も強い** |

**a を「対策」と呼ばない。** サニタイズは却下した — 落とすべき文字列の一覧は作れないし、
**作れたつもりになるのが一番危ない**。コメントを削ると LLM がいちばん使う材料が減る。

**出力の検証は「断る」であって「直す」ではない**（決定3）。解析 → **`base_iri` の外に
定義していないか** → SHACL の 3 つ。**2 つ目が生成の経路にだけある検査である** —
`publish` は主語の IRI 空間を検査していない（人が書いた TTL には外部語彙について述べる
自由を残した）が、生成された候補には課す。モデルが他の名前空間の `base_iri` の下に
用語を作れてしまうと、マッピングの所有者解決（`P2B-10`）がその用語を別の名前空間の
ものとして扱う。

**自動で直さない。** 補正はモデルの意図の推測であり、推測が `draft` に入るとレビュアは
「モデルの出力」ではなく「こちらの推測」を読むことになる。代わりに**検証の理由をそのまま
添えて再試行**し（既定 3 回）、上限に達したら 422 で断る — **部分的に正しい Turtle を
返さない**（決定4）。**何回目で通ったかを応答と監査に残す。**

**対象が多すぎたら 413 で断る。切り詰めない**（決定6）。切り詰めるとスキーマの一部しか
覆っていないオントロジーができ、**出力のどこにも「一部である」と書かれない**
（[ADR-0034](adr/0034-construct-describe.md) 決定4 と同じ「封筒が無い」問題）。

**実行は `owner`**（決定7）。`publish` は `data-steward` で足りるが、これは
**組織のスキーマのメタデータをモデルの提供者へ送る**外向きの操作で、**呼ぶたびに
課金される**。ソースの登録（ADR-0041 決定8）と同じロールに揃えた。

**API キーを使わない**（決定9）。`disableLocalAuth: true` でアカウントのローカル認証を
無効にし、マネージド ID（`Cognitive Services OpenAI User`）だけで呼ぶ。
**キーが存在しなければ、キーが漏れることもない。**

**既定のモデルは実測して決めた**（決定11。2026-09-12 / Japan East）。
...
既定は `gpt-4.1` / `GlobalStandard` / 容量 10（測った割り当ての約 0.1%）。

**`kind: 'AIServices'`（Foundry の新しい形）ではなく `kind: 'OpenAI'` を選んだ。**
`properties.endpoint` がそのまま推論の基底になるのがこの kind で最も確実で、
**1 回のデプロイ窓で確かめきるために確実さを取った**。実機で確認できたら寄せてよい。

**実機で確かめること**（デプロイ窓）:

| 何を | なぜローカルで確かめられないか |
|---|---|
| モデルのデプロイが作られる（割り当て・SKU） | ARM と割り当ての実際 |
| マネージド ID で推論が呼べる（キー認証は無効） | Entra 認証の実際 |
| 実モデルが**検証を通る Turtle を返すか**（試行回数） | プロンプトの品質はモデルでしか測れない |
| `MODEL_ENDPOINT` の形が `{endpoint}/openai/deployments/...` で合っているか | kind ごとの実際 |

### `P2A-19` スキャンの定期実行（ACA Job）

- **状態**: **実装は完了（2026-09-12）。実機確認はデプロイ窓で行う**
- **優先**: 中
- **設計**: [ADR-0042](adr/0042-scan-job.md)（決定 8 件）
- **出典**: `P2A-01` の設計（ADR-0041 決定10）で意図的に範囲外にした

**中心の判断は「既定で動かない」である**（ADR-0042 決定1）。`SCAN_JOB_CRON` が
空なら `Manual` トリガで、**自動では 1 度も走らない**。定期実行は他人の DB へ
繰り返し接続を試み、実行ごとに課金され、運用者が気づかないうちに始まる。
頻度は「そのソースがどれだけ変わるか」と「どれだけ負荷をかけてよいか」で
決まる — **こちらが知らないこと**である。`SCAN_ALLOWED_HOSTS` を既定で
空にしたのと同じ向きの判断。

**掃引の対象は登録済みの全ソース**で、名前空間で絞らない（決定2）。
`reconcile` と同じ**システムの保守処理**の位置にある。**トリガに引数が無いのは
意図** — 対象を選べる口を作ると「誰がどの DB をスキャンできるか」が名前空間の
権限の外に出る（ADR-0041 決定8 が避けたこと）。

**終了コードは「掃引が走ったか」しか語らない**（決定3）。1 つの顧客 DB が
落ちていることをジョブの失敗にすると、ACA の再試行が健全なソースを何度も
叩く。届かなかったことは `scan_runs` に `failed` として残る。**ただし
「0」を「全部うまくいった」と読ませない** — 失敗が 1 件でもあれば警告と
件数を出し、読むべき場所（`scan_runs`）を言う。

**ジョブは Core API と同じイメージを別のコマンドで動かす**（決定5）。
**Fuseki と Blob の資格情報は渡していない**（決定6）— スキャンが触るのは
PostgreSQL とソース DB だけで、**イメージが同じである以上、渡せば使えて
しまう**。`packages/api/tests/test_scan_job_infra.py` がこれをテキストで
固定する（`FUSEKI_ADMIN_PASSWORD` を env に足すと落ちる。実際に落ちることを
確認した）。

**`P2A-01` が残していた穴が 1 つ見つかった。** `SCAN_ALLOWED_HOSTS` と
`SCAN_VAULT_URL` を **Bicep に配線していなかった** — 実装もテストも揃って
いたのに、**デプロイ環境では allowlist が空のままで、ソースの登録が必ず
403 になる**状態だった。ローカルの `.env` では設定できていたので、
ローカルのテストでは気づけない。ADR-0042 を書いていて気づいた。

**azd がジョブのイメージを更新するかは確認できなかった。** `azd show` は
`host: not-a-real-host` すら黙って受け付ける（実測）ので、**受け付けたことは
対応の証拠にならない**。未確認の挙動に依存せず、`postdeploy` が
`az containerapp job update` で差し替えて**読み戻して確認する**（決定5）。

**実機で確かめること**（デプロイ窓）:

| 何を | なぜローカルで確かめられないか |
|---|---|
| ジョブが起動し、`Succeeded` で終わる | ACA の実行環境が要る |
| `postdeploy` がイメージを差し替える | `SERVICE_API_IMAGE_NAME` は azd が deploy 後に設定する |
| マネージド ID で PostgreSQL に届く | Entra 認証の PostgreSQL が要る |
| `SCAN_JOB_CRON` を設定すると `Schedule` になる | ARM の適用結果を見る必要がある |
...
- ~~**主体を `prov:Person` / `prov:SoftwareAgent` に分けない**~~（決定3）。**`P2A-16` で条件付きで出すようになった**（2026-09-12、[ADR-0035](adr/0035-actor-type.md)）。判断は変わっていない — **記録していないものは主張しない**。変わったのは「記録するようにした」ことである。**四眼原則を記録する監査証跡で、人間の承認を自動化された行為として見せるのは最悪の誤りである**
...
**却下したもの**: `Accept: text/turtle` で `/audit` に内容交渉させる（JSON には `cursor` が要るが RDF では意味が薄く、**同じ URL が表現によって違うパラメータを取る**形になる）/ `prov:wasRevisionOf` を「版の連番」として出す（外部ツールは派生として読む。説明は機械に届かない）/ ~~JSON-LD で返す~~（`@context` の公開先という新しい問題が生まれる → **`P2A-17` で「公開しない」ことで消した**。2026-09-12、[ADR-0036](adr/0036-jsonld-serialization.md)）/ 監査イベントを PROV-O のまま PostgreSQL に保存する（ADR-0006 が既に答えている）。

**未解決**: **カーソルページングが無い**（大きな名前空間は `since` / `until` で期間を区切る）/ **IRI が `urn:` なので dereferenceable ではない**（外部ツールから「この版の詳細を引く」ことはできない）/ 派生関係は `P2A-15` で、主体の種別は `P2A-16` で、JSON-LD は `P2A-17` で解決した（いずれも 2026-09-12）。
...
- **行とトリプルを混ぜない**（決定7）。`returned_row_count` を NULL 可にし、`returned_triple_count` を足した。**`0` を書かない**。~~`ASK` の既存の振る舞いは変えていない~~（**`P2B-22` で解決した**。2026-09-12、[ADR-0039](adr/0039-ask-row-count.md)）
...
**未解決**: 応答全体を解析する（`arq:queryTimeout` が事実上の防波堤）/ 上限を超えると結果が得られない / 空白ノードのラベルが保たれない / ~~**`CONSTRUCT` の結果に廃止済み用語の警告を出していない**~~（**`P2B-23` で解決した**。2026-09-12、[ADR-0038](adr/0038-rdf-deprecation-warning.md)）/ `DESCRIBE` の内容はストア依存（SPARQL 1.1 が規定していない）。
...
`P2A-16` の補足（**完了。2026-09-12**）: 設計は [ADR-0035](adr/0035-actor-type.md)。ADR-0026 決定3 と ADR-0006 補記 3 に補記を追加した。

**既存の実装は既に推測していて、その推測は成り立たなかった。** `Principal.is_service_principal` は `claims.get("idtyp") == "app" or "oid" not in claims` で導出されていた（どこからも読まれていなかった）。`idtyp` は**任意クレーム**で、リソース側のアプリ登録に設定しないと発行されない。設定していないテナントではサービスプリンシパルのトークンにも付かず、そのとき最後の砦になる `"oid" not in claims` は**アプリ専用トークンにも `oid` が付くため成り立たない**。結果としてこの式は、`idtyp` 未設定のテナントのサービスプリンシパルを**人間として判定する** — ADR-0026 決定3 が「最悪の誤り」と呼んだものの逆向きである。

**3 値にした**（決定1）。`user` / `service-principal` / `unknown`。真偽値にすると「クレームが無い」が「人間である」に潰れる。`AUTH_MODE=disabled` の `Principal.local_dev()` も `unknown` である（検証したトークンが無いので人間だと名乗る根拠が無い。副作用としてローカル開発の書き出しが「`idtyp` 未設定のテナント」と同じ見た目になり、運用者が設定漏れの状態を手元で見られる）。

**列は 1 本で足りた**（決定3）。`NULL` = 問うていない（この機能より前の行）、`'unknown'` = 問うて、分からなかった。[ADR-0027](adr/0027-revision-lineage.md) 決定1 が2 本必要だったのは `edited_from` が「親の版」という値の列で `NULL` に 2 つの意味が同居したためで、こちらは最初から列挙型なので `unknown` を語彙の中に持てる。

**種別は行為ごとに記録し、主体のクラスは矛盾しないときだけ出す**（決定4）。主体の IRI は主体ごとなので、種別を主体に付けると `idtyp` を設定する前と後の行為が同じ書き出しに混ざったときに 1 つの IRI へ複数の値がぶら下がる。行為ごとの記録は**測った事実**、主体のクラスは**主体についての主張**であり、主張には一致が要る。

**`AuditRepository.record` だけ `actor_type` を必須にした**（決定6）。「署名の狭さが強制である」（`P2B-20` の教訓）を正本の表に書く 1 箇所に効かせる。**実際に mypy が `retire` の記録漏れを 1 件捕まえた。** サービス層とルータには `unknown` の既定値を置いた（安全な向き — 渡し忘れは情報を落とすだけで偽の主張はしない）ので、ルータの渡し忘れは `packages/api/tests/test_actor_type_api.py` が記録経路の**網羅**で塞いでいる。

**`idtyp` の設定手順を製品に持った**（決定7）。`just setup-app-role` が `optionalClaims` を合成する（`appRoles` と同じ複合プロパティで、部分更新すると既存の定義が丸ごと消える。`P1-09` の罠1）。**設定しなければこの機能は永久に `unknown` を記録する。**

**未解決**: ~~`idtyp: "user"` が v2.0 のアクセストークンに乗るかを実機で確かめていない~~（**`P2A-18` で確認した。2026-09-13。乗る** — [ADR-0035 補記1](adr/0035-actor-type.md)。Entra の文書は `include_user_token` を「v1.0 固有の任意クレーム」節に載せているが、**実際には v2.0 でも効く**。したがって人間の行為は `user` として記録され `prov:Person` が出る）。**`device` を `unknown` に畳んでいる**ので、デバイストークンと設定漏れは区別できない（決定2。答えたい問いは「人間が説明責任を負うか」であり、どちらも「負う人がいない」）。**主体のクラスが書き出しの範囲に依存する**（決定4。期間で切った書き出しと全件の書き出しで `prov:Person` が付くかどうかが変わりうる。行為ごとの `ont:actorType` は変わらない）。

`P2A-17` の補足（**完了。2026-09-12**）: 設計は [ADR-0036](adr/0036-jsonld-serialization.md)。ADR-0026 決定1 と ADR-0031 に補記を追加した。

**保留の理由だった「`@context` をどこで公開するか」は、公開しないことで消えた。**文書に埋め込む（JSON-LD 1.1 の正規の形）。外に置く案はいずれも**「解決できる URL を名乗っているのに解決できない」**形になる — この API のコンテキスト URL は認証が要り（JSON-LD プロセッサは Bearer トークンを持たない）、デプロイごとに違い、`production` プロファイルは内部 ingress なので最初から解決できない。リポジトリの raw URL に置けば、文書の意味が GitHub の可用性とリポジトリ名に依存する。

**同じ URL の内容交渉で返す**（決定1）。`Accept: application/ld+json`。**ADR-0026 決定1 が却下した内容交渉とは別物である** — あちらは `/audit`（JSON のページ）と RDF の交渉で、理由は「同じ URL が表現によって違うパラメータを取る」ことだった。同じグラフの 2 つの直列化はパラメータが同じなので当たらない。

**内容交渉は足すだけにした**（決定5）。既定は Turtle で、`application/json` では切り替えず、解釈できない `Accept` でも 406 にしない。`q=0`（明示的な拒否）は尊重する — 部分文字列一致で判定すると**拒否を肯定として読む**。

**用語の別名を作らなかった**（決定3）。キーは `ont:action` のまま。別名は**2 つ目の語彙**であり、Turtle の述語名と同期させ続ける義務が生まれて情報は1 つも増えない。

**2 つの直列化が同じグラフから出ることを構造で保証した**（決定6）。`provenance_graph()` / `mapping_graph()` を切り出し、同型であることを`rdflib.compare.isomorphic` で固定した。

**RDF を返す口はすべて同じ規則にした**（決定8）。`/provenance` と `/mappings/export` の両方。片方だけにすると「なぜマッピングは JSON-LD で取れないのか」に技術的な答えが無い（どちらも同じ `Graph` である）。

**変異テストで 3 つの穴が出た**（20 件中 16 件を即座に kill）。(1) `graph.serialize` の `ensure_ascii` は最後の `json.dumps` に上書きされる**死んだ設定**だった（消した）。(2) 空のグラフには rdflib 自身が `"@graph": []` を出すので「空なら空配列」という**到達しない分岐**を書いていた（消して、依存している実測をテストで固定した）。(3) **コンテキストを取り違えても同型性は壊れない**（圧縮しか変わらない）ので、ルータが渡すコンテキストを別に固定する必要があった。

**未解決**: **`urn:` の IRI は dereferenceable でないままである**（`P2A-07` の未解決のまま）。コンテキストの埋め込みが解くのは「語彙の意味をどこで引くか」であって「主体や版の詳細をどこで引くか」ではない。**コンテキストが応答ごとに繰り返される**（1 応答あたり数百バイト。解決できないコンテキストはキャッシュもできない）。**`@graph` の配列である**ので、単一のオブジェクトを期待する JSONクライアントは `@graph` を辿る必要がある（平らにはできない — 1 つの書き出しに束・行為・主体・版という複数の主語が出る）。**`Accept` の解析は RFC 9110 の完全な実装ではない**（見るのは 2 つの型だけ）。
...
`P2A-14` の補足（**完了。2026-09-12**。`P2A-08` の実測で発見された）: 当時 `CONSTRUCT` / `DESCRIBE` はガードで 400 で断っており、対応には 3 つの設計が必要だった — (1) `FusekiStore.query` の内容交渉、(2) 戻り値の型、(3) **トリプル数の上限**（行数の上限とは別の量である）。いずれも [ADR-0034](adr/0034-construct-describe.md) で決めて実装した（上限超過は切り詰めずに 413 で断る — **「RDF には封筒が無い」**ため、切り詰めたグラフは切り詰めたと名乗れない）。
...
| `P2B-09` | 意味的差分の計算と差分レビュー | 決定 7 | 中 | **完了**（計算は 2026-09-10、**画面は 2026-09-13 の `P2A-03`**） |
...
| `P2B-21` | 廃止された先を指すマッピングの数を健全性指標に出す | — | 低 | **完了**（2026-09-12） |
| `P2B-22` | `ASK` のアクセスログが `returned_row_count = 0` を記録する | — | 低 | **完了**（2026-09-12） |
| `P2B-23` | `CONSTRUCT` / `DESCRIBE` の結果にも廃止済み用語の警告を出す | — | 低 | **完了**（2026-09-12） |
...
**却下したもの**: 権限に関わらず生死を返す（**`active` と `absent` を区別すると存在の oracle になる**。区別を捨てれば oracle は狭まるが、今度は決定2 が禁じた丸めになる。**権限で切るのが、どちらも犠牲にしない唯一の線**）/ 廃止された先を指すマッピングを自動で取り消す（ADR-0023 決定3 の裏返しをこちらが行うことになる。そして決定7 のとおり正当にありうる）/ 廃止済みの先への宣言を拒否する（決定3 の理由）/ `M` の承認をブロックする（人質）/ ~~健全性指標に足す~~（**指標が他の名前空間の Blob の可用性に依存する** → **`P2B-21` で「項目を `null` にせず、調べられなかった分を `unknown` として数える」形で解決した**。2026-09-12、[ADR-0037](adr/0037-deprecated-target-metric.md)）。
...
| ID | 内容 | 優先 | 状態 |
|---|---|---|---|
| `P3-01` | Ontop VKG（R2RML 管理 + 仮想グラフの照会） | 高 | **完了**（2026-09-13） |
| `P3-02` | 用語のベクトル検索（`search_context` ツール） | 高 | **完了**（2026-09-13。**AI Search は採らなかった**） |
| `P3-03` | Metric Service | 中 | 未着手 |
| `P3-04` | Context Manager のオーケストレーション | 高 | 未着手 |
| `P3-05` | 定義と実データの乖離検出 | 中 | **完了**（2026-09-13） |
| `P3-06` | （任意）Microsoft Purview コネクタ | 低 | 未着手 |
| `P3-07` | Ontop を Container Apps へ載せる | 高 | 未着手（**要デプロイ窓**） |
| `P3-08` | 仮想グラフの照会をアクセスログに残す | 中 | **完了**（2026-09-13） |
| `P3-09` | 乖離の定期実行と通知 | 低 | 未着手 |
...
`P3-05` の補足（**完了時に書き換えた**）: **定期実行は入れなかった**（`P3-09` に分けた）。報告を返す口を先に作った。詳細は下の節。

---

### `P3-01` Ontop VKG（**完了。2026-09-13**）

設計は [ADR-0046](adr/0046-virtual-knowledge-graph.md)（決定 16 件）。
**これが Phase 3 の起点で、[ADR-0002](adr/0002-triple-store-as-rebuildable-projection.md)
が成立する前提でもある** — 実データをグラフに載せないから EmptyDir の
4〜8 GiB で足りる、という判断がここに依存している。

**設計の前に実物を測った。推測で決めた項目は 1 つも無い。**

| 測ったこと | 結果 |
|---|---|
| 公式イメージの JDBC ドライバ | **1 つも無い**（`/opt/ontop/jdbc` すら無い）。`P1-S3` の結論を 5.3.0 で再確認 |
| TBox の推論 | **効く**（`Customer ⊑ Party` の上位クラスでインスタンスが返る） |
| TBox のトリプル自体 | **返らない**（`?s a owl:Class` は 0 件） |
| SPARQL Update | **受け付けない**（415。`/update` 系も 404） |
| 不正なマッピング | **コンテナが起動しない**（即時初期化のとき） |
| そのエラー本文 | **接続ユーザから見える全関係を列挙する**（制御平面の表も入っていた） |
| CA 証書 | **1 枚も無い**（イメージの中から TLS を検証できない） |

**いちばん重要な測定は「TBox のトリプルは返らない」である。** これは
**宛先を間違えると例外ではなく 0 件が返る**ことを意味し、0 件は
「該当する行が無い」と区別できない。だから

- 宛先を URL で分けた（語彙は Fuseki、実データは Ontop。決定1）
- **マッピングが無いソースへの照会は 404**（決定11）
- **エンドポイントが未設定なら 503**（決定11）

**2 つ目は「エラー本文が全関係を列挙する」である。** `VirtualGraphClient` は
本文をログにだけ出し、応答にはステータスまでしか載せない（決定10）。
`FusekiStore` が本文を 500 文字含めるのを直さないのは、**相手が違う**から
である（あちらは自分の射影先、こちらは顧客の DB）。

**主な決定**:

- **`rr:sqlQuery` を受け付けない**（決定4）。**パーサが無い。**
  `referenced_relations` は正規表現で、自身の docstring が「完全な SQL
  パーサではない」と書いている。任意の SQL に向けると**境界があるように
  見えて無い**状態を作る。本当の境界は DB ユーザの権限である
- **主語の IRI を名前空間のデータ接頭辞の下に閉じ込める**（決定5、不変条件5）。
  接頭辞は `base_iri` から導くので**新しい列を作らない**
- **自分の語彙だけ実在を検査する**（決定6）。外部語彙（`rdfs:label` 等）は
  検査しない。実在しない用語を通すと、**実データは流れているのに照会が
  0 件を返す**
- **マッピングは版に紐づけない**（決定12）。紐づけると**承認のたびに
  実データへの入口が止まる**。代わりに照合した版を記録し、`NULL` は
  「承認済み版が無かった」として残す
- **`rr:graph` / `rr:parentTriplesMap` / 空白ノードの主語 / 動的な述語を
  受け付けない**（決定6〜9）

**実機確認は `just test-vkg`（`containers/ontop/ontop-check.test.sh`）が行う。**
PostgreSQL を使い捨てで立て、**SPARQL の結合が SQL に書き換わって実データが
返ること**を確かめる。CI の `vkg` ジョブで毎回走る。

テストは Python が 1261 → **1343 件**（`test_r2rml.py` 31 /
`test_vkg_client.py` 12 / `test_vkg_api.py` 27 / `test_ontop_contract.py` 12）、
シェル側の検査が 6 項目。

**変異テストで 26 件の変異を当て、2 件が生き残った。** どちらも
テストの穴だった —
**「正本が読めなかった」を「用語は実在する」にしても落ちなかった**こと、
**退役した名前空間でマッピングを改訂できても落ちなかった**こと。
テストを 3 件足して両方を殺した。

**同梱したドライバは pgjdbc（BSD-2-Clause）と mssql-jdbc（MIT）だけ**で、
どちらも jar と POM の一次情報でライセンスを確認し、SHA-256 を Dockerfile に
固定した。

**確かめていないこと**:

- **デプロイ環境での動作は `P3-07`**
- **Entra トークン認証のソースで仮想グラフを立てられるか**（決定の帰結に
  「未検証」として記録した）。Ontop は起動時に資格情報を 1 度読む形で、
  トークンの期限切れに追従する仕組みが無い
...
### `P3-02` 用語のベクトル検索（**完了。2026-09-13**）

設計は [ADR-0050](adr/0050-vector-search-in-postgres.md)（決定 11 件）。
**標題が「Azure AI Search 統合」から変わった。** ID は変えていない。

**当初の計画（[ADR-0008](adr/0008-independent-implementation.md) の対応表）は
AWS 版の OpenSearch Serverless に対して Azure AI Search を当てる前提だった。
実装の前にその前提を測り直し、覆した。**
...
| Free tier の容量 / 休止 | 50 MB・索引 3 つ。**使われないと削除されうる** |
| Serverless Developer | プレビュー |
| Azure PG 16 の `vector` / `pg_trgm` | **どちらもある**（0.8.2 / 1.6）。**拡張の有効化に課金は無い** |
| `pg_bigm` / `pgroonga` | **無い**（日本語の形態素解析が使えない） |
| pgvector の `hnsw` の次元の上限 | **2000**（2001 以上で `column cannot have more than 2000 dimensions`） |
| Japan East の `text-embedding-3-small` v1 | 利用可。`GlobalStandard` の割り当ては上限 4,000 / 使用 0 |
| Japan East の `text-embedding-3-large` | **GlobalStandard が既に 3,002/6,000 使用中** |

**ベクトル検索を正本の PostgreSQL（pgvector）に載せた。追加費用 $0 である。**
[ADR-0043 決定9](adr/0043-ontology-proposal.md) が「キーが存在しなければ、
キーが漏れることもない」として推論のローカル認証を無効にしたので、
**検索のためにキーを 1 本増やすのはその判断の逆**だという点も決め手になった。

**2 つの経路を持つ。片方では成立しないことを実測で確かめた。**

| 問い | `similarity`（既定 0.3） | `word_similarity` | 判定 |
|---|---|---|---|
| 「顧客ID」 | **0 件** | `customerId` に 1.000 | 表記は 3-gram で当たる |
| 「お客様」 | 0.000 | **0.000**（閾値 0 でも当たらない） | **ベクトルの経路でしか当たらない** |
| 「注文日」 | 0 件 | `orderedAt` に 0.750 | — |

**主な決定**:

- **`similarity` ではなく `word_similarity` を使う**（決定6）。閾値の既定は
  **0.6 — PostgreSQL 自身が `<%` に対して選んだ値**である（実測。
  `similarity_threshold` は 0.3、`strict_word_similarity_threshold` は 0.5）。
  **緩めない** — 0.3 にすると「顧客ID」で `Customer` まで当たり（0.400）、
  それは**概念の隣**であってベクトルの経路の担当である
- **演算子（`<%`）ではなく明示的な比較で絞る。** `<%` はセッション変数を
  見るので、**設定した閾値が効いているかを応答から確かめられない**
- **埋め込みは射影で、承認の経路から呼ばない**（決定4、不変条件1・3）。
  独立した口（`POST .../search/rebuild`、`maintainer`）にした。
  **名前空間の行を丸ごと入れ替える** — 縮めた版（不変条件8）を反映した
  ときに「検索に出るが版には無い用語」を作らない
- **`text-embedding-3-small`（1536 次元）に固定する**（決定5）。3-large は
  3072 次元で `hnsw` の上限を超え、`halfvec` の式索引が必要になる
- **「ベクトルが使えない」を「該当なし」にしない**（決定8）。3-gram だけで
  結果を返し、`vector_available` / `vector_note` / `embedded_term_count` /
  `conclusive` を必ず添える。**モデルが落ちても検索は失敗させない**
- **廃止済みの用語も返す**（決定9、ADR-0017 決定3 と同じ向き）
- **MCP に `search_context` を出す。作り直しは出さない**（決定11）。
  MCP は読み取り専用で、**費用が発生する行為をエージェントに開けない**

**実装で踏んだ罠（CLAUDE.md に追記した）**:

- **SQLAlchemy の `text()` で `%` を二重にしてはいけない。** asyncpg の
  プレースホルダは `$1` 形式なのでエスケープされず、`%%` と書くと
  `operator does not exist: text %% text` で落ちる
- **`Base.metadata.create_all` は `__table_args__` に書いた索引しか作らない。**
  マイグレーションの生の `CREATE INDEX` だけでは、**テストの DB（create_all）
  と本番の DB（alembic）で索引が違う**状態になる。索引の存在を確かめる
  テストが落ちて気づいた。ORM 側に `postgresql_using` / `postgresql_ops` で
  宣言して揃えた
- **`SHOW pg_trgm.*` は同じセッションで pg_trgm の関数を呼んでおかないと
  `unrecognized configuration parameter` になる**
- **`EmbeddingClient` の `__aenter__` / `__aexit__` が何もしない見せかけ
  だった。** `async with` を読んだ側は閉じたと思うが、実体は呼び出しごとに
  `httpx.AsyncClient` を作り捨てていた。`VirtualGraphClient` と同じ形
  （1 本持って `aclose` で閉じる）に直した — **作り直しは 64 件ずつ何十回も
  呼ぶので、毎回 TLS を握り直すのは実害である**

**測っていないこと（`P3-10` に分けた）**:

- **索引が planner に選ばれるか。** 212 行では `hnsw` も `gin` も選ばれず
  Seq Scan になる（実測）。**索引が「ある」ことは固定したが、「使われる」
  ことは固定していない**
- **実モデルでの検証。** 埋め込みの生成には Azure OpenAI のデプロイが必要で、
  デプロイ窓の対象である
- **hnsw の作り直しの所要時間**と、**B1ms を検索と正本で共有する影響**

---

### `P3-05` 定義と実データの乖離検出（**完了。2026-09-13**）

設計は [ADR-0047](adr/0047-definition-data-divergence.md)（決定 10 件）。
**`P3-01` が終わって初めて可能になった項目である。**

**何を見つけるものか。** 承認したオントロジーは「こういう形のデータがある」
という主張である。**その主張が実データと食い違っても、今までは誰も
気づかなかった** — SHACL 検証（`P2A-05`）が見るのは投入された TTL 自身で、
スキーマだけの TTL には対象ノードが無いので「適合」を返す。

**pyshacl を実データに当てない**（決定1）。当てるにはデータをメモリに載せる
必要があり、それは [ADR-0002](adr/0002-triple-store-as-rebuildable-projection.md)
と [ADR-0046](adr/0046-virtual-knowledge-graph.md) が守っているもの
（実データを実体化しない）を**検査のために壊す**ことになる。

代わりに **`sh:NodeShape` から SPARQL の探りを組み立てて仮想グラフに問う**。
返るのは `ASK` の真偽と `COUNT` の 1 行だけで、**実データは 1 行も
こちらに来ない**。

**いちばん重要な決定は「0 件を 4 状態に分ける」である**（決定2）。

| 何が起きたか | 状態 | 意味 |
|---|---|---|
| 実データが 1 件以上あった | `matched` | — |
| マッピングはあるのに 0 件 | `empty` | **これが乖離** |
| マッピングがそのクラスを作っていない | `unmapped` | **乖離ではない。** 入口が無い |
| 照会できなかった | `unknown` | **何も言えない** |

**3 つを 1 つの `0` に潰すと、マッピングの書き忘れも Ontop の停止も
「実データが無い」として報告される。** 運用者が取るべき対処（マッピングを
書く / Ontop を直す / 定義を狭める）がまったく違うのに、見分けがつかない。

**実装の途中で推測が 1 つ外れた。**

| 書き方 | Ontop 5.3.0 |
|---|---|
| `FILTER NOT EXISTS { ?s p ?v }` | **拒否される**（`OntopUnsupportedKGQueryException` で 500） |
| `OPTIONAL { ?s p ?v } FILTER(!BOUND(?v))` | **動く** |
| `MINUS { ?s p ?v }` | **動く** |

最初は `NOT EXISTS` で書いていて、**Python 側のテストは全部通っていた**
（探りの文字列を固定するだけなので）。**実物に当てて初めて分かった。**
`NOT EXISTS` のほうが素直に読めるので「読みやすくする」変更で戻されうる —
だから **Python のテストで `"NOT EXISTS" not in query` を固定し、シェルの
検査で実物が 500 を返すことも固定した**（後者が本体。Ontop が対応したら
そのテストが落ちて前提が変わったと分かる）。

**ほかの決定**:

- **測れないときは空の報告を返さない**（決定6）。承認済み版が無い・
  マッピングが無い・正本が読めないは 404、エンドポイント未設定は 503、
  退役済みは 409。**形が 1 つも無いときだけ 200 + `no_shapes`** —
  形が無いのは異常ではないが、「乖離が無い」とは区別する
- **探りの本数に上限を置き、超えた分を `unknown` として報告に入れる**
  （決定5）。欠落させると**「載っていないものは問題なし」と読まれる**
- **クラスに実データが無ければプロパティの探りを投げない**（決定5）。
  無駄な顧客 DB へのクエリを打たない。**投げなかった分は `issued_probes`
  から引く**
- **乖離を承認のゲートにしない**（決定10）。乖離は「定義が悪い」とは
  限らない（実データがまだ無い新しいクラスは正当な `empty`）。加えて
  **不変条件15 に触る** — 承認の可否を顧客 DB の可用性に依存させない
- **定期実行は入れなかった**（決定9、`P3-09`）。`scan_job` と同じ判断
  （既定で動かない）が必要で、**通知の宛先も決まっていない**

**変異テストで 22 件の変異を当て、全件殺した。**

Python のテストが 1343 → **1395 件**（`test_divergence.py` 38 /
`test_divergence_service.py` 14）。シェル側の検査は 6 → **7 項目**。
...
- **大きなテーブルに対する探りのコスト。** `ASK` は 1 行で止まるが、
  ...
  測っていない**（題材は 5 行である）
- **`sh:path` がプロパティパスの形**（`sh:inversePath` 等）は必須プロパティ
  として拾わない
...
- **状態**: 未着手
- **優先**: 低
- **内容**: `P3-05` の乖離の測定を定期的に走らせ、結果を運用者に届ける
- **なぜ `P3-05` でやらなかったか**: 2 つ決まっていないことがある。
  - **既定で動かすかどうか。** `scan_job`（[ADR-0042](adr/0042-scan-job.md)
    決定1）は Manual トリガを既定にした。**探りは顧客 DB へのクエリなので、
    同じ判断が必要である**
  - **通知の宛先。** 誰に届けるのかが決まっていない。名前空間の `owner`？
    `term_owners`？ **宛先を決めずに定期実行だけ足すと、
    「動いているのに誰も見ていない」状態を作る**
- **出典**: `P3-05` の設計（ADR-0047 決定9）で意図的に範囲外にした

---

### `P3-08` 仮想グラフの照会をアクセスログに残す（**完了。2026-09-13**）

設計は [ADR-0048](adr/0048-vkg-access-log.md)（決定 7 件）。
**[ADR-0046](adr/0046-virtual-knowledge-graph.md) 決定13 を撤回した**
（あちらに補記1 を書いた）。

**この経路が返すのは顧客の実データである。** この製品でいちばん機密性の
高い経路が、監査証跡に残っていなかった。

ADR-0046 決定13 は理由を **`access_events` の形が合わないこと**（版の概念が
無い）としていた。**形が合わないことは記録しない理由にならない** — 形を
直せばよかった。

**列を 2 つ足した。既存の行の意味は変えていない**（決定1）。

| `vkg_source` | 何の照会か |
|---|---|
| `NULL` | **オントロジーへの照会**。**これは既存の全行と同じ意味である** |
| 非 `NULL` | そのソースの仮想グラフへの照会 |

**列を足すときに既存の行の読み方が変わってはいけない。** 変わると
**過去の記録が信用できなくなる**（「この行は新しい意味か古い意味か」を
後から判定できない）。

**この列があるから `default_graph_version` の `NULL` が曖昧でなくなった**
（決定2）。`NULL` かつ `vkg_source` が `NULL` なら「承認済み版が無かった」、
`vkg_source` が非 `NULL` なら「版の概念が無い」である。
`vkg_mapping_revision` が版の代わりになる（「どの定義の入口を通したか」）。

**主な決定**:

- **`term_access` は更新しない**（決定3）。あの表は**用語数で上限される**
  ことが設計の一部で、インスタンスの IRI を入れると
  **エージェントの稼働に比例して行が増える**。しかも `returned_terms` は
  前方一致なので、**`base_iri` がスラッシュ区切りならデータ IRI が用語として
  数えられる**。**`build_vkg_access_record` は `terms` 引数を持たない** —
  コメントで禁じるのではなく、渡せないようにした
- **絞り込みは 2 つの別の引数**（決定4）。`vkg_source=<名前>` と
  `ontology_only=true`。**`vkg_source=None` を「オントロジーだけ」と
  解釈すると、絞り込まない既定と区別できない**
- **切り詰める前の行数を記録する**（決定5、ADR-0025 決定6 と同じ）。
  `CONSTRUCT` はトリプル数を記録する — **ADR-0046 決定14 は上限を当てないと
  決めたが、数えないとは決めていない**（補記2 を書いた）
- **記録に失敗しても照会は失敗させない**（決定6）。ただし警告は残す
- **仮想グラフのクライアントを依存として注入した**（決定7）。
  **理由はテスト可能性である** — ハンドラの中で作ると**アクセスログを
  記録する経路を差し替えられない**

**変異テストで 13 件の変異を当て、全件殺した。**
...
**受け入れたコスト**: **クエリ本文が残る。** 既存の行と同じ扱い（2,000 文字で
切り詰め + 全文のハッシュ）だが、**仮想グラフのクエリには顧客データの値が
入りうる**（`FILTER(?name = "田中 太郎")` のような形）。新しい問題では
ないが、**仮想グラフでは値が実データになるので影響は大きい**。保持ポリシーと
「`owner` だけが読める」で扱う。

---

### `P3-07` Ontop を Container Apps へ載せる

- **状態**: 未着手（**要デプロイ窓**）
- **優先**: 高
- **内容**: `containers/ontop` のイメージを ACA の内部 ingress に載せ、
  マッピングを API から取って起動する経路（init コンテナ）を作る
- **設計で決める必要があること**:
  - **Ontop の 1 インスタンスは 1 つの DB しか見ない**（`--db-url` が 1 つ）。
    ソースが増えるとコンテナが増える。「1 ソース = 1 Container App」を
    どう表現するかを決める必要がある
  - **Entra トークン認証のソースを扱えるか。** Ontop は起動時に資格情報を
    1 度読む形で、期限切れに追従する仕組みが無い。`ONTOP_DB_PASSWORD_FILE`
    を読み直すかは**確かめていない** — 読み直したとしても、プール内の
    既存接続は古い資格情報で作られたままである
  - `VKG_ENDPOINT_TEMPLATE` に何を入れるか（内部 ingress の FQDN）
- **なぜ今やらないか**: デプロイ窓が必要（`azd up` 約 15 分 +
  `azd down --purge` 約 27 分）。**ローカルで確かめられることは
  `P3-01` で全部確かめた**
- **出典**: `P3-01` の設計（ADR-0046）で意図的に範囲外にした

#### 2026-09-13 に測ったこと（着手前の調査。**実装はまだ無い**）

`P3-02` の作業中に、**設計を決めるために足りていなかった事実**を実測した。

| 測ったこと | 結果 |
|---|---|
| `ontop/ontop:5.3.0` の基盤 | **Ubuntu 22.04.5 LTS**。`apt-get` がある |
| HTTP クライアント | **`wget` はある**（`curl` は無い）。`nc` / `perl` もある |
| CA 証書 | **無い**（`P3-01` の実測どおり）。**ただし基盤が Ubuntu なので `apt-get install ca-certificates` で足せる** — ADR-0046 は「イメージの中から TLS を検証できない」という事実を記録しただけで、足せないとは書いていない。**足すという選択肢が検討されていなかった** |
| Java | **JRE 11**（Temurin 11.0.26）。`javac` / `jshell` は無い（`jrunscript` はある） |
| 実行ユーザ | `ontop`（uid 999） |
| API がソースの接続先を返すか | **返す**。`ScanSource` が host / port / database / username / auth_mode / **`vault_secret_name`（名前だけ）** を持つ。**パスワードは返さない**（ADR-0041 決定4） |

**これで設計の骨は決まる。**

1. **マッピングと接続先は起動時に Core API から取る。** `GET .../vkg-mapping.ttl`
   （`P3-01` でこの口を作った理由がここで効く）と `GET .../scan-sources/{name}`。
   **内部 ingress は http なので TLS は要らない**。トークンは IMDS（http）から取る
2. **パスワードだけが Key Vault にある。** 2 案:
   (a) Container Apps の `secrets` に `keyVaultUrl` で解決させる（`fuseki.bicep` の
   `fuseki-admin-password` と同じ形。**秘密名を provision 時に知る必要がある**）
   (b) 起動スクリプトが Key Vault の REST を叩く（**CA 証書を足す必要がある**）。
   **(b) を採る見込み** — (a) は「顧客 DB の在り処」を PostgreSQL と Bicep の
   2 か所に持つことになり、**食い違いうる**。(b) なら正本は PostgreSQL のままである
3. **`auth_mode: entra` のソースは扱えない。** Ontop は資格情報を起動時に 1 度
   読むだけで、期限切れに追従しない。**起動を拒否して理由を言う**のが正しい
   （起動して後のクエリで失敗すると、「実データが無い」と見分けがつかない）
4. **init コンテナは使わない。** `fuseki.bicep` が使っていない理由と同じ —
   **azd deploy はメインコンテナのイメージしか差し替えない**ので、init の
   タグが `azd up` 一回では揃わない
5. **Ontop は状態を持たないのでレプリカ 0 にできる。** Fuseki を 1 に固定して
   いるのは射影の書き込み経路のためで、Ontop は純粋なクエリ変換器である。
   **`minReplicas: 0` なら待機中は $0**（初回クエリが遅くなる代償）。
   「minimal で評価できる」を掲げているので既定は 0 が妥当

**未解決のまま残る論点**（ここが `P3-07` の本体である）:

- **azd は 1 サービス = 1 コンテナアプリである。** Bicep のループで
  インスタンスを増やせても、**`azd-service-name` は 1 つのアプリしか指せない**
  ため、動的な集合にイメージを配れない。**「1 つだけ deploy する」か
  「イメージの配布経路を azd の外に出す」かの判断が要る**
- Ontop の MI に**名前空間のロール（`data-analyst`）を付与する運用手順**が
  必要になる（不変条件11 のとおり既定は拒否なので、付与しないと読めない）。
  `azd up` だけで完結しないことを受け入れるか、postdeploy で付与するか

---

### `P3-12` 識別子の並び順が照合順序に依存していた（**完了。2026-09-13**）

- **状態**: **完了**（`P3-02` と同じコミット）
- **優先**: 高
- **出典**: `P3-02` でローカルの PostgreSQL のイメージを
  `postgres:16-alpine` から `pgvector/pgvector:pg16` へ替えたときに、
  **触っていない `test_term_owners.py` の並び順のテストが落ちた**

**イメージの差し替えが壊したのではない。既にあった食い違いを見えるように
しただけである。**
...
| **Azure Database for PostgreSQL Flexible Server** | **`en_US.utf8`**（読み取り専用。他の値を選べない。Microsoft Learn の「Preset Options」で確認） |
| `infra/modules/postgres.bicep` の宣言 | `en_US.utf8` |

**つまりデプロイ環境はずっと `en_US.utf8` で、ローカルと CI だけが `C`
だった。** 落ちたテストは**本番では成り立たない順序を主張していた** —
`GET /namespaces/{ns}/term-owners` は Azure 上では別の順序で返っていた。

**実測した差**（`en_US.utf8` は第一水準で句読点を無視する）:
...
**IRI だけの問題ではない。** 名前空間名は `[a-z0-9-]` なので該当する。
...
**直した内容**: `ontology_core.db.by_identifier()` を足し、**識別子の列を
並べるときに `COLLATE "C"` を指定する**ようにした（用語 IRI・名前空間名・
プリンシパル ID・マッピングの両端・ソース名・スキーマ名/表名）。

- **列の照合順序は変えない。** 変えると既存のデプロイをマイグレーションで
  触る必要があり、索引も作り直しになる。**並べ替えは読み取りの都合**なので、
  読むときに指定するほうが影響が小さい
- **人間が読むテキスト（ラベル・説明）には当てない。** バイト順に並べると
  **日本語の並びが壊れる**。当てていないことを
  `test_identifier_ordering.py` が機械的に検査する
- **`C` に寄せた理由**: 識別子は言語ではなく、**クライアントが自分で
  並べ替えた結果（Python の `sorted()`、JavaScript の `Array.sort()`。
  どちらもコードポイント順）と一致してほしい**

**イメージは戻さない。** `en_US.utf8` は Azure と同じであり、**ローカルが
...
- **状態**: 未着手
- **優先**: 低
- **内容**: 用語数を増やしたときに `hnsw` と `gin` が planner に選ばれるか、
  作り直しの所要時間、B1ms を検索と正本で共有する影響を測る
- **なぜ必要か**: **`P3-02` は「索引がある」ことしか固定していない。**
  212 行では planner がどちらの索引も選ばず Seq Scan になる（実測）。
  行数が少ないからだが、**「索引が効いている」ことは確かめていない**。
  「測れなかった」を「問題なし」と書かないために記録する
- **測る対象**: 1 名前空間あたり 1,000 / 10,000 / 50,000 用語での
  ①検索の応答時間 ②`EXPLAIN ANALYZE` の索引の選択 ③作り直しの所要時間
  ④作り直し中の正本への書き込みの遅延
- **出典**: `P3-02` の実装（ADR-0050「失ったもの / 引き受けたリスク」）

---

### `P3-11` 埋め込みの作り直し漏れを検出する

- **状態**: 未着手
- **優先**: 中
- **内容**: 承認済み版と埋め込みの `built_from_version` が食い違っている
  名前空間を見つけて報告する。健全性指標か `reconcile` の報告に載せる
- **なぜ必要か**: **`P3-02` は明示的な作り直しの口しか持たない**
  （ADR-0050 決定4。承認の中で呼ぶと**埋め込みモデルの停止が承認を止める**）。
  つまり**版を承認して作り直しを忘れると、検索が古い版の用語を返し続ける**。
  `vector_available` は真のままなので、**「使えている」ように見える**のが
  この抜けの悪いところである
- **設計で決める必要があること**:
  - 報告の置き場所（`GET /namespaces/{ns}/health` の指標か、
    `POST /admin/reconcile` の報告か）。**射影の乖離を扱っているのは
    後者**（[ADR-0013](adr/0013-reconcile-repairs-observed-divergence.md)）
  - **自動で作り直すか。** 直すと費用が発生するので、**既定では報告だけ**が
    妥当と見ている（`reconcile` が Fuseki を直すのとは性質が違う）
- **出典**: `P3-02` の設計（ADR-0050 決定4 の却下した案）
...
`docs/third-party-licenses.md` が「**新しいコピーレフト依存が別の名前で
入ってきた場合は捕まえられません**」と書いていた穴を埋めた。
`scripts/check-licenses.py` が **Python 83 件 + Node 205 件 = 288 件**を
許諾リストと突き合わせる（`just check-licenses`、CI の `python` ジョブ）。

**いちばん重要な決定は「判定できなかったものを通さない」である**（決定1）。
実測でライセンスの綴りは揺れていた（同じ Apache-2.0 が 4 通り。
**`pyshacl` は `License` 欄に全文 11,491 文字**）。綴りを寄せる表に無いものは
`unknown` として**検査を落とす** — 「知らない綴りだから通す」を 1 度でも
許すと、**この検査は「緑なのに何も検証していない」状態になる**。

**その性質が実装中に効いた。** `Mozilla Public License 2.0 (MPL 2.0)` が
`unknown` になり、**それで自分のバグに気づいた**（`strip("()")` が末尾の
`)` だけを落として別名の照合を外していた）。回帰テストを置いた。

**ほかの決定**:

- **許諾リストを広げるのは人の判断**（決定2）。スクリプトが学習しては
  いけない。広げたら理由を `docs/third-party-licenses.md` に書く
- **SPDX の複合式を意味どおりに評価する**（決定3）。`OR` はどちらか一方、
  `AND` は両方。**`OR` と `AND` を混ぜた式は人に回す**（優先順位の解釈が
  要るので推測で通さない）
- **`License` 欄の全文を読まず分類子へ回す**（決定4）。全文から識別子を
  当てるのは推測になる
- **検査を実行できなかったことと検査の結果を分ける**（決定5）。`pnpm` が
  無いときは**終了コード 2**。`0` を返すと「ライセンスは全部確認済み」として
  CI が緑になる
- **Java 側とコンテナの基盤イメージは範囲外。範囲外を明記する**（決定6）。
  「CI が通ったから全部確認済み」と読まれないため

**実データでガードが発火することを手で確かめた**（`MIT` を許諾リストから
外すと 42 件・終了コード 1、戻すと 0）。テストは 1403 → **1441 件**。

**確かめていないこと**: `pnpm licenses list` は `apps/web` で実行している
...
`P2B-21` の補足（**完了。2026-09-12**）: 設計は [ADR-0037](adr/0037-deprecated-target-metric.md)。ADR-0030 と ADR-0020 に補記を追加した。

**却下の理由は、ADR-0020 決定3 が既に答えていた形だった。** 「指標が他の名前空間の Blob の可用性に依存する」ことは障害ではない — Blob の可用性に依存する項目は既にある（`term_count` / `shacl_violation_count`）。

**本当に新しい問題は「呼び出し元によって値が変わる」ことだった。** マッピングの先の生死は相手の名前空間への権限に依存する（ADR-0030 決定1）。`ns` に `data-analyst` を持つ 2 人が、他の名前空間への権限が違えば違う数を見る。**ただしこれは新しい依存ではない** — 一覧（`GET /namespaces/{ns}/mappings`）が既に呼び出し元の権限に依存して `target_status` を返している。

**2 つで 1 組にした**（決定1）。`deprecated_target_mapping_count` だけを見せると「残りのマッピングは健全」と読める。`unknown_target_mapping_count` を必ず並べる。**これは ADR-0020 決定3 の `null` と同じ判断を、項目の中の粒度で行うものである** — あちらは「項目が測れたか」、こちらは「マッピング 1 件ごとに調べられたか」。細かいぶん**測れた分を返しつつ測れなかった分を数えられる。**

**この 2 項目は `null` にならない**（決定2）。`int | None` で宣言すると**到達しない分岐**を作ることになる（そこにコメントを書くと、動かない振る舞いを説明する嘘になる）。`HealthReport` の型注釈が `int` であることをテストで固定した。

**`outgoing` だけを数える**（決定3）。`incoming` は**自分では直せない** — 行動に結びつかない数字を健全性の欄に置かない。廃止する側への報告は ADR-0030 決定7（`MAPPED_BY_OTHERS`）が別に持っている。

**指標のために権限ゲートを外さなかった**（決定4）。サービス自身の権限で全名前空間を読めば客観的な数になるが、**存在の oracle が開く** — 自分が張ったマッピングは自分で選べるので、1 件ずつ張って数の増減を見れば相手の語彙を探れる（集約しても閉じない）。逆に呼び出し元の権限で数える限り**新しい情報は 1 つも増えない**ので、ADR-0020 決定4（`data-analyst` が読める）を変えずに足せた。

**副産物**: `MappingRepository.disputed_count` を消した。`outgoing()` を2 回叩くことになるため、`HealthService` が 1 回だけ取って両方を数える形にした（`_decorate` が相手側の宣言を突き合わせるので安くない）。

**未解決**: **指標が呼び出し元に依存する**（「この数字は誰が見た数字か」を意識しなければならない）。**Blob の読み込みが最大 11 倍になりうる**（自分の TTL 1 件 + `MAX_TARGET_NAMESPACES` = 10 件）。上限は ADR-0030 決定4 のものをそのまま使う — 経路ごとに変えると一覧と指標で `unknown` の件数が食い違う。**どのマッピングが廃止を指しているかは指標では分からない**（件数だけ。一覧で引く）。**`absent`（相手の現行版から消えている）は数えていない** — 4 状態のうち廃止だけを数えている（項目を増やしすぎない判断）。

`P2B-23` の補足（**完了。2026-09-12**）: 設計は [ADR-0038](adr/0038-rdf-deprecation-warning.md)。ADR-0017 決定3 と ADR-0034 に補記を追加した。

**穴の形**: 同じデータが、クエリの形を変えるだけで警告なしに取れていた。`SELECT ?s WHERE { ?s a owl:Class }` では警告が出るのに、`CONSTRUCT { ?s ?p ?o } WHERE { ?s a owl:Class ; ?p ?o }` では出ない。ADR-0017 決定3 が塞いだ穴が**別の入口で開いたまま**だった。しかも `CONSTRUCT` は「自分のストアへ読み込む」用途で使われるので、**廃止済み用語がそのまま利用者のストアへ複製される**。

**切り詰めとは問題の形が違う**（ADR-0038 のコンテキスト）。ADR-0034 決定4 が切り詰めを 413 で断ったのは「**ヘッダでは足りないから**」であって「ヘッダが悪いから」ではない。切り詰めは**グラフが不完全になる**ので、ヘッダに書いても**不完全なグラフが完全なものとして届く**。廃止の警告は**グラフが完全で正しい**うえでの助言なので、Core API の層ではヘッダで足りる（エージェント向けには MCP が本文へ載せ替える）。

**RDF 側のほうが網羅的である**（決定4）。`deprecated_iris_in_results` は束縛のセルしか見ないので**述語を拾えない**（`SELECT ?s ?o` の結果に述語は現れない）。グラフからなら主語・述語・目的語のすべてを見られる。**同じ意図のクエリで警告の数が変わる**のは受け入れるコストである（多いほうが安全側）。

**解析が 3 回ではなく 1 回になった**（決定3）。この経路は上限の検査（`load_construct_result`）とアクセスログの用語（`terms_in_graph`）で**同じ本文を 2 回解析していた**。廃止の検査を足せば 3 回になるので、`RdfResult.iris` に IRI の集合を持たせて 1 回に減らした。`terms_in_graph` は**解析しない純粋な絞り込み** `terms_with_prefix` に置き換えた。

**`base_iri` で絞らない**（決定6）。廃止済み用語の集合はそのデータセットに対する SPARQL で求めているので、そこに入るのは呼び出し元が既に読める用語だけである。絞ると**取り込んだ外部語彙の廃止を警告できなくなる**。

**踏んだ罠**: **FastAPI のハンドラが `Response` を返すと、注入された `Response` のヘッダは消える**（`fastapi/routing.py` が `raw_response` をそのまま使い、注入側を合流させない。実測で確認し、CLAUDE.md に記録した）。注入側に載せる実装を書きかけたが、**ヘッダが黙って消える**。回帰テストで固定した。

**変異テスト**: 12 件中 11 件を即座に kill。生き残った 1 件は本当の穴だった — アクセスログの用語を `base_iri` で絞らない変異（`terms_in_graph` を置き換えたときに絞り込みが呼び出し側へ移ったため、それを見るテストが無かった）。テストを足して 12 件すべてを kill。

**未解決**: **ヘッダは Core API の層でしか届かない**（素の HTTP クライアントで `CONSTRUCT` を叩く利用者は、ヘッダを見なければ気づかない）。これは `SELECT` と同じ性質であり、エージェントのための層（MCP）では本文に載せてある。**廃止済み用語の取得が 1 クエリ増える**（`SELECT` の経路では既に払っている）。

`P2B-22` の補足（**完了。2026-09-12**）: 設計は [ADR-0039](adr/0039-ask-row-count.md)。ADR-0034 決定7 と ADR-0018 決定1 に補記を追加した。

**これは `ASK` の問題ではなかった。** `_row_count` は**形が読めなかったときにも `0` を返していた** — `ASK` はそれが日常的に起きる経路にすぎない。ストアを差し替えられる設計（ADR-0001）なので、持ち込みストアが想定外の形を返したときにも同じ誤りが起きる。**`ASK` だけを分岐で特別扱いすると、`0` を作っている本体が残る。**

直したのは「**束縛の配列が見つからなければ `None`**」という 1 点である（決定1・2）。結果として**行の概念が無いものすべてが `NULL` になり、規則が 1 つになった**。

| クエリの形 | `returned_row_count` | `returned_triple_count` |
|---|---|---|
| `SELECT` | 行数（**0 行も測った `0`**） | `NULL` |
| `ASK` | `NULL` | `NULL` |
| `CONSTRUCT` / `DESCRIBE` | `NULL` | トリプル数 |
| 読めない形 | `NULL`（+ `SELECT` なら警告ログ） | — |

**空の束縛の `0` は残した**（決定1）。「`SELECT` が 0 行返した」は測った事実であり、区別しているのはまさにここである。`None` に丸めると「何も返さないクエリ」を見つけられなくなる。

**列を足さなかった**（決定3）。`ASK` の真偽値は**記録したクエリと版で再実行できる**（ADR-0018 決定1 が用語の一覧を保存しなかったのと同じ理由）。クエリの形は `query_text` の先頭に残る（切り詰めは 2,000 文字なので形のキーワードは消えない）。**記録済みのものに名前を付けるだけの列は、食い違ったときにどちらが正しいのか分からなくなる。**

**既存の行は書き換えていない**（決定4）。行から形を判別するには `query_text` の解析が要り、**正本を推測で書き換える**ことになる。そして `0` と記録された行には**本当に 0 行だった `SELECT`** が混ざっている。ADR-0027 決定4 / ADR-0035 決定3 と同じ判断である。マイグレーションは不要（0009 で既に `NULL` 可）。

**新しい `NULL` の理由を黙って作らなかった**（決定5）。`SELECT` なのに行数が `NULL` になったときだけ警告ログを出す（`ASK` の `NULL` は正常なので出さない）。以前は読めない形でも `0` を記録していたので、**この劣化は `0` に紛れて見えなかった**。

変異テスト 7 件すべてを kill（生き残りなし）。

**未解決**: **`NULL` に 2 つの理由が同居する**（「行という概念が無い」と「読めなかった」）。`query_text` の形で区別できるので列は足さないが、**列だけを見ると区別できない**。**この変更より前の `ASK` の行は `0` のまま**なので、集計するときは期間で切る必要がある。

`P2B-13` の補足（**完了。2026-09-12**）: 最後に残っていた判断の設計は [ADR-0040](adr/0040-term-owner-not-an-approval-gate.md)。ADR-0010 と ADR-0015 に補記を追加した。

**承認の条件はこれで確定である。** `maintainer` 以上（ADR-0014）、四眼原則（ADR-0010 決定4）、受け入れ基準の出自（ADR-0029）の 3 つだけである。`term_owners` は入らない。

**代わりに差分レビューで問い合わせ先を返す**（決定2）。`GET /namespaces/{ns}/versions/{v}/diff` の応答に `owners` が入る。**ADR-0015 が「得られるもの」に挙げていて未実装だった約束**である（「差分レビューのルーティング先が決まる」）。`source` が `term-owner` / `namespace-owners` / `unresolved` のどれかを必ず言う — 代替で解決したことを隠さない（ADR-0015 決定2）。

**報告はするが止めない**（決定3）。`unresolved` があっても `diff` は成功し、`approve` も止まらない。止める仕組みにした瞬間に決定1 のジレンマに戻る。

**差分に載った用語についてだけ解決する**（決定4）。全用語を解決すると、`truncated` が真のときに「一覧に無い用語の問い合わせ先」が並び、どの用語のものか分からなくなる（上限も別に必要になる）。**`deprecated_terms` を対象に含める理由は `modified_terms` が `null` になる場合にある** — 空白ノードが 300 個を超える版では `modified_terms` が計算されないので、そのとき廃止を伝えるのは `deprecated_terms` だけである（**この経路は変異テストで見つけた**）。

**まとめて解決する口を足した**（決定5）。`resolve_many` はクエリ 2 回で、用語数に依らない（用語ごとに引くと 50 用語で最大 100 クエリになる）。`resolve`（1 件）はそれに委譲する — **規則を 2 か所に書かない**ため（フォールバックの順序が片方だけ変わると、1 件で引いたときと差分で見たときで問い合わせ先が違うという静かな不整合になる）。クエリ数はテストで数えている。

**応答の形を揃えた。** 基準が無い版（最初の版）でも `owners` のキーは入る（空の配列）。形が入力で変わる契約はクライアントに分岐を強いる。

**踏んだ罠**: 空白ノードを増やすテスト用の TTL に SHACL の property shape を使ったところ、**`approve` の SHACL 検証が「shapes 自体が不正」で落ちた**（不正な形状を作っていた）。独自述語の空白ノードに変えた。

変異テスト 11 件中 10 件を kill。残った 1 件（`dict.fromkeys` で重複をまとめない）は**等価変異**である — 返す辞書のキーが自然に重複を落とすので出力が変わらない。テストの説明をそう書き直した（**「重複排除を固定している」という誤った主張をやめた**）。

**未解決**: **責任者の承認は強制されない**（規制対応の文脈で「用語の責任者が署名した」ことを機械的には保証しない。保証するのは四眼原則と受け入れ基準の出自である）。**問い合わせ先を見たことは記録に残らない**（`owners` は監査に保存しない。残すなら `reason` に書く）。**責任者への通知経路は未解決のまま**（ADR-0015 の未解決事項。`owners` は問い合わせ先を返すだけで届けはしない）。
