...
| `P2A-15` | `base_version` を保存する | `P2A-07` の未解決。**保存すれば `prov:wasDerivedFrom` を測った事実として出せる**（いまは承認の順序しか知らないので出していない） |
...
| `P2A-14` | `CONSTRUCT` / `DESCRIBE` に対応する | 低 | 未着手 |
| `P2A-15` | `base_version` を保存する（派生関係を出せるようにする） | 中 | 未着手 |
...
**未解決**: **カーソルページングが無い**（大きな名前空間は `since` / `until` で期間を区切る）/ **IRI が `urn:` なので dereferenceable ではない**（外部ツールから「この版の詳細を引く」ことはできない）/ 派生関係（`P2A-15`）/ 主体の種別（`P2A-16`）/ JSON-LD（`P2A-17`）。
...
`P2A-15` の補足（**未着手**。`P2A-07` で発見）: `publish` が受け取る `base_version` を保存していない（lost update の検出にだけ使っている。`P1-13`）。保存すれば `prov:wasDerivedFrom` を**測った事実として**出せる。マイグレーションと、**既存の版には値が無い**ことの扱い（`NULL` を「分からない」として扱い、派生を出さない）が要る。
