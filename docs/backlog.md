...
Phase 2 で完了したもの: `P2A-05`（SHACL 検証）/ `P2A-06`（名前空間 RBAC と四眼原則）/ `P2A-09`（`platform-admin` の設定の自動化。**実機確認は未了**）/ `P2B-02`（保持ポリシー）/ `P2B-03`（廃止のライフサイクル）/ `P2B-04`（用語単位の責任者）/ `P2B-05`（アクセスログ）/ `P2B-07`（想定質問の CI 実行）/ `P2B-08`（`reason` の記録と参照）/ `P2B-09`（意味的差分の計算）/ `P2B-11`（監査を読み出す API）。

**次に着手する候補**（費用が発生しないものを優先順に）:

| ID | 内容 | なぜ次か |
|---|---|---|
| `P2B-06` | 健全性指標の集計と提示 | **6 項目すべての原資料が揃った**（`P2B-04` で責任者、`P2B-05` で参照、`P2B-09` で削除された IRI、SHACL 違反と未射影は既存）。あとは集計するだけ |
