...
| `P2A-19` | スキャンの定期実行（ACA Job） | **デプロイ窓（課金）。** Bicep は `az bicep build` で構文までしか確かめられない（ADR-0041 決定10） |
...
| ACA Job が起動し、マネージド ID で Key Vault とソース DB に届くか | `P2A-19` |
...
| `P2A-19` | スキャンの定期実行（ACA Job の Bicep） | 中 | 未着手（**要デプロイ窓**） |
...
- **状態**: 未着手（**要デプロイ窓**）
- **優先**: 中
- **内容**: `POST /namespaces/{ns}/scan-sources/{name}/scan` は同期で走る。
  大きなスキーマだと HTTP のタイムアウトに当たりうるし、定期実行もできない。
  ACA Job（`scan-job`）で非同期に回せるようにする。**API 側の契約は既にある**
  （`scan_runs` が `running` / `succeeded` / `failed` を持つので、Job が
  `ScanService` をそのまま呼べる）
- **なぜ今やらないか**: **Bicep は課金なしに動作を検証できない。**
  `az bicep build` で確かめられるのは構文までで、Job が起動するか・マネージド ID で
  Key Vault に届くか・ネットワークからソース DB へ到達できるかは**実機でしか
  分からない**。「書いたが動かしていない IaC」を残すのはこのリポジトリが
  避けている形である（[ADR-0041](adr/0041-source-schema-scan.md) 決定10）
