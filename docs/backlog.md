...
最終更新: 2026-09-12（`P2A-02` の実装後。**柱 A の実装はすべて終わった** — 残るのは**実機確認**（デプロイ窓）と**Web UI**（`P2A-03` / `P2A-04`）である）
...
| `P2A-18` | `idtyp: "user"` が実トークンに乗るかの確認 | **デプロイ窓（課金）。** `P2A-09` の窓で同時に確かめられる |
...
| `idtyp: "user"` が v2.0 のアクセストークンに乗るか | `P2A-18` |
...
| `P2A-02` | LLM によるオントロジー候補生成（OWL/SHACL） | 高 | **実装は完了**（2026-09-12。**実機確認はデプロイ窓で**） |
...
| `P2A-18` | `idtyp: "user"` が v2.0 のアクセストークンに乗るかの実機確認 | 低 | 未着手（**要デプロイ窓**） |
...
| `P2A-19` | スキャンの定期実行（ACA Job の Bicep） | 中 | **実装は完了**（2026-09-12。**実機確認はデプロイ窓で**） |
...
**未解決**: **`idtyp: "user"` が v2.0 のアクセストークンに乗るかを実機で確かめていない**（`P2A-18`）。`include_user_token` 追加プロパティは Entra の文書では「v1.0 固有の任意クレーム」節の下にある。効かなければ人間は永久に `unknown` で、`prov:Person` は出ない（設計はその場合も正しい — 偽の主張はしない）。**`device` を `unknown` に畳んでいる**ので、デバイストークンと設定漏れは区別できない（決定2。答えたい問いは「人間が説明責任を負うか」であり、どちらも「負う人がいない」）。**主体のクラスが書き出しの範囲に依存する**（決定4。期間で切った書き出しと全件の書き出しで `prov:Person` が付くかどうかが変わりうる。行為ごとの `ont:actorType` は変わらない）。
