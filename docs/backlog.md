...
最終更新: 2026-09-13（**Phase 2 が完了した**。柱 A・柱 B とも実装・実機確認が済んでいる。残るのは `P2A-20`（Web の MSAL 認証。要デプロイ窓）と Phase 3 である）
...
| `P2A-20` | Web の認証を MSAL にする | **デプロイ窓（課金）。** SPA のリダイレクト URI はアプリ登録の変更を伴う |
...
| `P2A-20` | Web の認証を MSAL にする | 中 | 未着手（**要デプロイ窓**） |
