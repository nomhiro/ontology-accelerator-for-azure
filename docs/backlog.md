...
最終更新: 2026-09-13（**Phase 3 を進めている**。`P3-01`（Ontop VKG）と `P3-05`（定義と実データの乖離検出）が完了。残るデプロイ窓は `P2A-21` と `P3-07` である）
...
Phase 3 で完了したもの: `P3-01`（Ontop VKG。**宛先を間違えると 0 件が返ることを実測したので、宛先を URL で分けた**）/ `P3-05`（定義と実データの乖離検出。**pyshacl を実データに当てず、形から SPARQL の探りを組み立てる**）。

**下の表は「こちらの判断だけでは進めないもの」である**（Phase 3 の残り`P3-02`〜`P3-06` と Phase 4 は別表にある）。`P2A-21` と `P3-07` は**ローカルで検証できない**（デプロイ窓が必要）、`P2A-22` は**消してよいことを確かめていない**、`P3-08` は**既存の行の意味を変えない設計が要る**。どれも、判断だけで進めると「緑なのに何も検証していない」状態を作る。
...
| `P2A-22` | ロックファイルが 2 つある（npm と pnpm） | **由来の確認。** `.gitignore` にも入っていないので、意図して置かれた可能性がある |
...
| `P3-08` | 仮想グラフの照会をアクセスログに残す | **設計。** `access_events` の既存の行の意味を変えずに列を足す方法を決める |
...
| `P2A-22` | ロックファイルが 2 つある（npm と pnpm） | 低 | 未着手 |
...
### `P2A-22` ロックファイルが 2 つある（npm と pnpm）

- **状態**: 未着手
- **優先**: 低
- **内容**: `apps/web/package-lock.json`（npm）と `pnpm-lock.yaml`（pnpm）が
  **両方コミットされている**。CI と `justfile` は pnpm しか使わないので、
  npm のロックファイルは**誰も読まないまま古くなる**
- **なぜ問題か**: 読まれないロックファイルは**嘘をつく**。`apps/web` で
  `npm install` した人は、CI と違う依存の版を手に入れる。
  **実測（2026-09-13）: `npm ci` はもう失敗する** —
  `npm error Missing: stackback@0.0.2 from lock file`。
  `package-lock.json` の最終コミットは 2026-08-28（`da37b7e`）で、
  その後に足した cytoscape と `@azure/msal-browser` の解決情報が無い。
  `vitest` は `devDependencies` に名前だけあって解決先が無い。
  **つまり「npm でも入る」と思って叩くと、何が起きたのか分からない
  エラーで止まる**
- **なぜまだ直していないか**: 消すだけで済むはずだが、**消してよいことを
  確かめていない**（`.gitignore` にも入っていないので、意図して置かれた
  可能性がある）。消す前に由来を確認したい
...
| `P3-08` | 仮想グラフの照会をアクセスログに残す | 中 | 未着手 |
...
### `P3-08` 仮想グラフの照会をアクセスログに残す

- **状態**: 未着手
- **優先**: 中
- **内容**: 仮想グラフへの照会を `access_events`（`P2B-05`、
  [ADR-0018](adr/0018-context-access-log.md)）に記録する
- **なぜ問題か**: **この経路が返すのは顧客の実データである。** それが
  監査証跡に残らない。「説明可能・監査可能」を掲げる製品で、いちばん
  機密性の高い経路だけ記録が無いのは筋が通らない
- **なぜ `P3-01` でやらなかったか**: `access_events` の行は「どの**版**の
  何を返したか」を持つ形で、仮想グラフには版が無い。
  `default_graph_version` を `NULL` で埋めると**「承認済み版が無かった」と
  混ざる**（ADR-0046 決定13）。列を足す変更は健全性指標・監査 API・
  `term_access` の更新に波及するので、範囲を分けた
- **今どうなっているか**: **アプリのログには出している**（actor / 名前空間 /
  ソース / 改訂 / 行数 / 切り詰め）。監査証跡ではないが、無記録でもない
- **設計で決める必要があること**: 列を足すか（`vkg_source` のような
  nullable な列。`NULL` = オントロジーへの照会 = 既存の全行と同じ意味）、
  別の表にするか。**既存の行の意味を変えてはいけない**
- **出典**: `P3-01` の設計（ADR-0046 決定13）で意図的に範囲外にした
