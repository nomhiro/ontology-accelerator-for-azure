...
| ID | 内容 | 必要なもの |
|---|---|---|
| `P1-S2` | ACA の idle/active 課金比率の実測 | 数時間〜数日デプロイを維持。**費用が発生する** |
| `P1-14` | POSIX 経路での `azd up` の通し確認 | 実機デプロイ 1 回。**費用が発生する** |
| `P1-25` | reconcile が欠落グラフを自動復旧するか | ADR での設計判断（`projected_at` の意味と `SUPERSEDED_RETAIN` との関係） |

`P1-S2` と `P1-14` は**同じデプロイ窓に相乗りさせる**（MCP 認証の実機再確認も一緒に行う）。実施前にユーザーの確認を取る。
