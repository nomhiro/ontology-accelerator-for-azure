...
| ID | 内容 | 必要なもの |
|---|---|---|
| `P1-25` | reconcile が欠落グラフを自動復旧するか | ADR での設計判断（`projected_at` の意味と `SUPERSEDED_RETAIN` との関係） |
| `P1-26` | 同一内容の再 publish が 201 Created を返す | 小さな修正（冪等な一致は 200 にする） |

**費用の出る作業は完了した。** `P1-S2`（idle/active 課金）・`P1-14`（POSIX 経路）・MCP 認証の実機再確認を 2026-09-09 の 1 つのデプロイ窓（約 55 分）で片付け、`azd down --purge` した。残る 2 件はローカルで完結する。
