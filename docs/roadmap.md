...
[`backlog.md`](backlog.md) の `P1-*` を参照。Critical 1 件を含む。
...
根拠は [ADR-0009](adr/0009-ontology-operations.md)。決定 1〜8 に対応する。

- 決定可能なものを機械が判定する層（構文検証 → SHACL → **OWL 推論器を CI へ**）
- 廃止のライフサイクル（`owl:deprecated` + 後継への参照。IRI は削除も再利用もしない）
- 名前空間と用語の責任者
- 健全性指標（未参照の用語、責任者未設定、再承認が古い、SHACL 違反、未射影）
- 想定質問（Competency Questions）を SPARQL テストとして CI で実行
- 「なぜ」を参照時に返す（`reason` / `diff` を書き、読み出す）
- 意味的差分の計算と差分レビュー
- 領域間マッピング（SKOS `closeMatch` 等）
