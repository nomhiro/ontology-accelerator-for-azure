...
- 顧客 DB 接続とスキーマ自動発見 — **完了**（2026-09-12。`P2A-01`、[ADR-0041](adr/0041-source-schema-scan.md)）。**実データを 1 行も読まない** — カタログは LLM のプロンプトへ流れるので、一度混ざったら消せない。発行する SQL を 5 本に固定し、テストが「`pg_catalog` / `information_schema` 以外を参照していない」ことを機械的に検査する。**定期実行（ACA Job）は `P2A-19`** — 課金なしに検証できない
