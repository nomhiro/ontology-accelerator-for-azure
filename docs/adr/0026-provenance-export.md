...
- [ADR-0036](0036-jsonld-serialization.md) — 決定1 に補記を追加した
  (**却下したのは `/audit` と RDF の交渉**であり、同じグラフの直列化の
  交渉ではない)
...
主体の種別(決定3)と JSON-LD(`P2A-17`)は、どちらも下の補記で解決した。
...
## 補記: JSON-LD を同じ URL の内容交渉で返すようになった (2026-09-12、`P2A-17`)

代替案の節は JSON-LD を却下していた。理由は

> `@context` をどこで公開するかという新しい問題が生まれる

だった。[ADR-0036](0036-jsonld-serialization.md) は**公開しないこと**でこの問題を
消した — `@context` を文書に埋め込む。外に置く案はいずれも「解決できる URL を
名乗っているのに解決できない」形になる(この API のコンテキスト URL は認証が
要り、デプロイごとに違う)。

**決定1 が却下した内容交渉とは別物である。** 決定1 が却下したのは `/audit`
(JSON のページ)と RDF の交渉で、理由は「**同じ URL が表現によって違う
パラメータを取る**」ことだった。`Accept: application/ld+json` で切り替えるのは
**同じグラフの 2 つの直列化**であり、パラメータは同じである。`/audit` と
`/provenance` の境界は変えていない。

`cursor` を受けない判断(決定1)も変えていない。JSON-LD でも受けない。

**切り詰めの旗は引き続き RDF の中にある**(決定5)。`Accept` を要求するのは
**クライアントが形を選ぶため**であり、安全に関わる情報をヘッダに載せたのでは
ない([ADR-0017](0017-deprecation-lifecycle.md) 決定3 の「エージェントは
ヘッダを見ない」は変わらない)。
