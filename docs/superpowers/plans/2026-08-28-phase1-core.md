...
**対応**: `FusekiStore.create_dataset` を、フォーム形式ではなく
**`Content-Type: text/turtle` で assembler 記述を POST する方式**に変える。
Fuseki の admin API はこの形式も受け付ける。渡す assembler は Step 1 の雛形と
**同一の内容**にすること(動的作成と再起動後の挙動を一致させるのが目的)。

雛形が1箇所に書かれている状態を保つため、`containers/fuseki/` のファイルと
Python 側で二重管理にならないようにすること。Python 側で組み立てる関数を作り、
ローダはそれを使えない(シェルなので)から複製が必要なら、**両方に「もう一方も直すこと」を
コメントで相互参照させる**。
