...
**注意:** `asyncpg` は `connect_args["password"]` に callable を受け付ける。受け付けない場合は
`sqlalchemy.event.listens_for(engine.sync_engine, "do_connect")` でトークンを注入する形に切り替える。
実装時にどちらが通るか確認し、通った方を採用してコメントで理由を残すこと。
...
async def test_create_is_rejected_when_duplicated(session: AsyncSession) -> None:
    repo = NamespaceRepository(session)
    kwargs = dict(
        display_name="x", description="", base_iri="https://e.example/#", created_by="t"
    )
...
class OntologyBlobStore:
    ...
    @classmethod
    def from_client(
        cls, service: BlobServiceClient, *, container: str, prefix: str
    ) -> Self:
        return cls(service.get_container_client(container), prefix=prefix)
...
async def test_version_is_incremented_for_new_content(prepared) -> None:  # noqa: ANN001
    ...
    first = await svc.publish(namespace="retail-core", turtle=TTL, actor="t")
    second = await svc.publish(namespace="retail-core", turtle=TTL + "\nex:B a ex:Class .\n", actor="t")
