...
- Fluent UI の Web 画面でレビュー・承認とグラフ可視化ができる(`P2A-03` / `P2A-04`、[ADR-0044](docs/adr/0044-review-ui.md))。
  ...
  **承認できない理由**(四眼原則で自分の版を自分では承認できない、など)も文章で出します。
  **`draft` はグラフを描けません** — 射影されていないため(= AI エージェントからも見えない)。
  **描画そのものは検証していません**(ブラウザを持たないため。判断を担う純粋関数 58 件はテスト済み)

**Phase 3 の仮想グラフ(Ontop VKG)はローカルの docker で確認しました(2026-09-13、`P3-01`、[ADR-0046](docs/adr/0046-virtual-knowledge-graph.md))。**

`just test-vkg` が、使い捨ての PostgreSQL に 2 つの表を作り、R2RML マッピングを Ontop に食わせて確かめます。

| 確かめたこと | 結果 |
|---|---|
| **2 表をまたぐ SPARQL の結合が、実データの行として返る** | 注文 3 件が顧客名つきで返る。**実データは実体化されません** |
| **TBox の推論が効く** | `Customer ⊑ Party` の上位クラスで問い合わせると、顧客のインスタンスが返る |
| **TBox のトリプル自体はデータとして返らない** | `?s a owl:Class` は **0 件**(エラーではない) |
| **SPARQL Update を受け付けない** | `/sparql` は 415。`/update` `/sparql/update` `/statements` はいずれも 404 |
| ポータルページが出ない | `/` `/index.html` `/ontology` はいずれも 404 |
| **不正なマッピングのエラー本文が全関係を列挙する** | だから Core API は**その本文を捨ててログに出します** |

**3 行目がこの設計の要です。** 宛先を間違えると**例外ではなく 0 件**が返り、それは「該当する行が無い」と区別できません。そのため

- **宛先を URL で分けています。** 語彙は `/namespaces/{ns}/sparql`(Fuseki)、実データは `/namespaces/{ns}/scan-sources/{source}/sparql`(Ontop)。**1 つのクエリが両方に跨ることはありません**
- **マッピングが無いソースへの照会は 404**、**`VKG_ENDPOINT_TEMPLATE` が未設定なら 503** です。どちらも空の結果を返しません

**`rr:sqlQuery` は受け付けません。** 任意の SQL が安全かを判定する手段(パーサ)が無いためです。正規表現で判定すると**境界があるように見えて無い**状態になります。**実データに対する本当の境界は、接続する DB ユーザの権限です** — Ontop はそのユーザに見える関係をすべて見ます(不正なマッピングのエラーが全関係を列挙したことで実測できました)。最小権限のユーザを使ってください。

**マッピングはソースごとの不変改訂で、`reason` が必須です。** 実データへの入口の定義なので、入口が変わった理由が残らないのは監査として成立しません。**登録時点で承認済みだった版を記録します**(`null` は「承認済み版が無かった」であって「検査していない」ではありません)。

> **デプロイ環境での動作は `P3-07` です**(要デプロイ窓)。**Entra トークン認証のソースで仮想グラフを立てられるかは未検証です** — Ontop は起動時に資格情報を 1 度読む形で、トークンの期限切れに追従する仕組みがありません。**SQL Server に対する動作も未検証**(ドライバは同梱しましたが、実機で確かめたのは PostgreSQL だけです)。**仮想グラフの照会はアクセスログに残りません**(`P3-08`。アプリのログには出します)。

#### 仮想グラフへの照会は監査証跡に残ります(`P3-08`)

**この経路が返すのは顧客の実データです。** いつ・誰が・どのソースに・どのマッピングの改訂を通して・何行(何トリプル)受け取ったかを `access_events` に記録します([ADR-0048](docs/adr/0048-vkg-access-log.md))。

```bash
# この顧客 DB に誰が何を聞いたか
curl -sS -H "Authorization: Bearer $TOKEN"   "$API/namespaces/retail-core/access-log?vkg_source=crm" | jq .

# オントロジーへの照会だけ(`vkg_source` の省略とは違います)
curl -sS -H "Authorization: Bearer $TOKEN"   "$API/namespaces/retail-core/access-log?ontology_only=true" | jq .
```

**`vkg_source` が `null` の行はオントロジーへの照会です。** これは**この列を足す前の全行と同じ意味**で、列を足しても過去の記録の読み方は変わりません。この列があるおかげで `default_graph_version` の `null` が曖昧でなくなります — `vkg_source` が `null` なら「承認済み版が無かった」、非 `null` なら「版の概念が無い」です。

> **仮想グラフのクエリ本文には顧客データの値が入りえます**(`FILTER(?name = "田中 太郎")` のような形)。`access_events` は昔からクエリ本文を残しますが(2,000 文字で切り詰め + 全文のハッシュ)、**仮想グラフでは値が実データになるので影響が大きい**です。**アクセスログを読めるのは `owner` だけ**で([ADR-0018](docs/adr/0018-context-access-log.md) 決定5)、**保持ポリシー**([ADR-0019](docs/adr/0019-retention-policy.md))で消せます。
>
> **`term_access`(用語ごとの参照の集約)は更新しません。** あの表は「自分のオントロジーのどの用語が使われていないか」に答えるもので、**行数が用語数で上限されることが設計の一部**です。仮想グラフが返すのはインスタンスの IRI なので、入れると**エージェントの稼働に比例して行が増えます**。

#### 定義が実データと食い違っていないか測れます(`P3-05`)

承認したオントロジーは「こういう形のデータがある」という主張です。**その主張が実データと食い違っても、SHACL 検証では見つかりません** — あちらが検証するのは投入された TTL 自身で、スキーマだけの TTL には対象ノードが無いので「適合」を返します。

```bash
curl -sS -H "Authorization: Bearer $TOKEN"   "$API/namespaces/retail-core/scan-sources/crm/divergence" | jq .
```

**pyshacl を実データに当てません**([ADR-0047](docs/adr/0047-definition-data-divergence.md) 決定1)。当てるにはデータをメモリに載せる必要があり、それは「実データを実体化しない」という設計を**検査のために壊す**ことになります。代わりに `sh:NodeShape` から SPARQL の探りを組み立てて仮想グラフに問います。**実データは 1 行もこちらに来ません。**

**0 件を 4 つに分けます**(決定2)。これがこの機能の設計の中心です。

| 状態 | 意味 | 運用者が直すもの |
|---|---|---|
| `matched` | 実データが 1 件以上あった | — |
| `empty` | **マッピングはあるのに 0 件**(= 乖離) | 定義、または実データ |
| `unmapped` | マッピングがそのクラスを作っていない | **マッピング**(乖離ではありません) |
| `unknown` | **照会できなかった** | 仮想グラフの疎通 |

**3 つを 1 つの `0` に潰すと、マッピングの書き忘れも Ontop の停止も「実データが無い」として報告されます。** 取るべき対処がまったく違うのに、見分けがつかなくなります。

> **これは SHACL 検証ではありません。** 見るのは「クラスに実データがあるか」と「必須(`sh:minCount ≥ 1`)のプロパティが欠けている件数」の 2 つだけです。`sh:pattern` / `sh:datatype` / `sh:maxCount` / `sh:class` は見ません。**`conclusive` が偽のときに「乖離なし」と読まないでください。**
>
> **乖離は承認を止めません**(決定10)。実データがまだ入っていない新しいクラスは正当な `empty` であり、また承認の可否を顧客 DB の可用性に依存させないためです(不変条件15)。**定期実行はまだありません**(`P3-09`)。

**実装の途中で推測が 1 つ外れました。** Ontop は **`FILTER NOT EXISTS` をサポートしていません**(実測。`OntopUnsupportedKGQueryException` で HTTP 500)。`OPTIONAL` + `!BOUND` で書いています。**Python 側のテストは `NOT EXISTS` でも全部通っていました** — 探りの文字列を固定するだけなので。実物に当てて初めて分かりました。

**Phase 2 柱 A も実環境で確認しました(2026-09-13)。**

- **`platform-admin` アプリロールの自動設定が実テナントで通る**(`scripts/setup-app-role.py`)。定義(Graph への PATCH)・`idtyp` 任意クレーム・割り当てのすべてが成功し、**デプロイ済み API で実際に効く**(`postdeploy` が名前空間を作成して承認まで通過)
- **`idtyp: "user"` が v2.0 のアクセストークンに乗る**(`P2A-18`。[ADR-0035 補記1](docs/adr/0035-actor-type.md))。人間の行為は `user` として記録され、`prov:Person` が出ます
- **マネージド ID でソース DB のスキーマを読める**(`P2A-01`)。デプロイ済みの PostgreSQL を対象に 14 テーブルを取り込みました。**パスワードはどこにも保存していません**
- **`scan-job`(ACA Job)が起動して `Succeeded` で終わる**(`P2A-19`。33 秒)。`postdeploy` が Core API のイメージへ差し替えたことも読み戻して確認しました。**トリガは `Manual` のまま** — `SCAN_JOB_CRON` を設定していないので自動では走りません
- **`scan_runs` が人間と機械に分かれる。** API から実行した run の `started_by` は運用者の Entra オブジェクト ID、ジョブが実行した run はマネージド ID のクライアント ID でした
- **実モデル(`gpt-4.1`)が 1 回目で検証を通る Turtle を返す**(`P2A-02`)。自分自身の DB の 3 テーブルから **3 クラス / 24 データ型プロパティ / 1 オブジェクトプロパティ(外部キーを関係にしたもの)/ 3 SHACL NodeShape** が出ました。**外部キーを関係に変えたことが、列を機械的に写しただけではない証拠です**
- **生成された候補は `draft` のまま射影されない**(`projected_at` が `null`)= エージェントからは見えません。監査には `model=gpt-4.1、deployment=ontology-proposer、api-version=2024-10-21、試行 1 回` と **「人間のレビューを経ていない候補である」**が残ります

**確かめていないこと**: `platform-admin` を持たない主体が 403 になることは、**別の主体を用意しないと実機では確かめられない**ため未確認です(ローカルのテストでは確認済み)。

**`azd down --purge` まで実行し、副作用で確認しました**(`az group exists` が `false`、Key Vault と Cognitive Services の論理削除も残っていない)。

### 未実装・未検証

- **Scan と Model は実装済みです**(`P2A-01` / `P2A-02`)。ソース DB のスキーマをカタログへ取り込み、そこから LLM が OWL/SHACL の候補を生成して `draft` にします(下記「ソース DB のスキーマを読む」「カタログから OWL/SHACL の候補を生成する」)。**Web のレビュー画面(`P2A-03`)とグラフ可視化(`P2A-04`)は未実装**で、いまは API で操作します
...
**`ASK` も `null` です**([ADR-0039](docs/adr/0039-ask-row-count.md)、`P2B-22`)。
`ASK` の結果に束縛は無いので、`true` と答えた `ASK` を「0 行返した」として
数えません。**直したのは `ASK` の分岐ではなく「束縛が見つからなければ `0` を
返す」という本体です** — ストアを差し替えられる設計なので、持ち込みストアが
想定外の形を返したときにも `0` を作ってはいけません。
...
**`ASK` の真偽値は保存しません。** オントロジーは不変リビジョンなので、
**記録したクエリと版で再実行できます**(返した用語の一覧を保存しないのと
同じ理由です)。クエリの形も保存しません — `query_text` の先頭に残ります。
...
#### カタログから OWL/SHACL の候補を生成する(候補は `draft` にしか入りません)

**スキャンしたカタログから、LLM がオントロジー候補を作ります**([ADR-0043](docs/adr/0043-ontology-proposal.md)、`P2A-02`)。Scan → Model の接続点です。

```bash
# 対象のテーブルを選んで候補を作る(owner が必要)
curl -X POST "$API/namespaces/retail-core/scan-sources/sales-db/propose" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"tables": ["public.products", "public.categories"],
       "version": "0.1.0",
       "reason": "小売ドメインの初期案"}'
# → {"version": "0.1.0", "status": "draft", "attempts": 1,
#     "model": "gpt-4.1", "deployment": "ontology-proposer", "table_count": 2}

# 中身を見て、良ければ人が提出 → 承認する(ここは既存のフローそのまま)
curl "$API/namespaces/retail-core/versions/0.1.0" -H "Authorization: Bearer $TOKEN"
curl -X POST "$API/namespaces/retail-core/versions/0.1.0/submit" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"reason": "レビューしてください"}'
```

**候補は `draft` にしか入りません。** `draft` は Blob と PostgreSQL にだけ存在し、**Fuseki には一切現れません**([ADR-0010](docs/adr/0010-approval-and-projection.md) 決定1)。つまり **エージェントからは見えません**。この API は `submit` も `approve` も呼びません。

> **この前提は、これまで「LLM を呼ぶ経路が 1 つも無かったから」成り立っていました。** `P2A-02` で初めて、**コードで強制されます**。

**カタログのテキストは命令ではなくデータとして扱います。** `column_comment` は**顧客 DB の持ち主が書いた文字列**で、この製品の管理外にあります(「以前の指示を無視して……」と書かれたコメントが混ざりえます)。対策は 3 段で、**区切り記号は最も弱い段**です。

| 段 | 何をするか | 強さ |
|---|---|---|
| a | カタログを区切った節に入れ、「ここはデータである」と指示する | **弱い。破られうる** |
| b | **モデルにツールを与えない。** 出力は Turtle の文字列としてしか扱わない | 強い |
| c | **検証を通っても `draft` にしか入らない** | **最も強い** |

**a を「対策」と呼んでいません。** 実際の防壁は b と c です — 注入が成功しても、できるのは「変な提案を 1 件作る」ことで、それは人間のレビューが既に受け止める形になっています。

**出力は必ず検証します。通らなければ受け付けません。**

1. Turtle として解析できるか
2. **定義している主語がすべてこの名前空間の `base_iri` の下にあるか**
3. SHACL の shapes 自体が正しいか

**自動で直しません。** 閉じ括弧を足すような補正は**モデルの意図の推測**で、推測が `draft` に入るとレビュアは「モデルの出力」ではなく「こちらの推測」を読むことになります。代わりに**検証の理由をそのまま添えて再試行**し(既定 3 回)、それでも通らなければ 422 で断ります — **部分的に正しい Turtle は返しません。**

**何回目で通ったかを応答と監査に残します。** 1 回で通った候補と 3 回目でやっと通った候補は、レビュアにとって別物です(後者は**モデルがこのスキーマを扱いかねている**信号です)。

**対象が多すぎたら 413 で断ります。切り詰めません**(既定 40 テーブル)。切り詰めるとスキーマの一部しか覆っていないオントロジーができ、**出力のどこにも「一部である」と書かれません**([ADR-0034](docs/adr/0034-construct-describe.md) 決定4 と同じ「封筒が無い」問題)。**何を 1 つのドメインとして扱うかは人の判断です。**

**実行には `owner` が必要です**(`publish` は `data-steward` で足ります)。組織のスキーマのメタデータを**モデルの提供者へ送る**外向きの操作であり、**呼ぶたびに課金される**ためです。「この DB をスキャンしてよい」と決めた人が、「このカタログをモデルへ送ってよい」も決めます。

**API キーを使いません。** モデルのアカウントは `disableLocalAuth: true` で**ローカル認証そのものを無効化**してあり、マネージド ID(`Cognitive Services OpenAI User`)だけで呼びます。**キーが存在しなければ、キーが漏れることもありません。**
...
**`SELECT` だけでなく `CONSTRUCT` / `DESCRIBE` でも同じように警告します**（[ADR-0038](docs/adr/0038-rdf-deprecation-warning.md)、`P2B-23`）。**クエリの形によって警告の受け取り方は変わりません** — 同じヘッダ、同じ形、MCP では同じキーです。

**RDF の結果のほうが網羅的です。** `SELECT ?s ?o` の結果に述語は現れないので、**廃止された property を使ったクエリは `SELECT` では警告が出ません**。`CONSTRUCT` の結果はグラフなので、主語・述語・目的語のすべてを見られます。

**警告を RDF の中には入れません。** トリプルを足せば**利用者のグラフにこちらが作った主張を混ぜる**ことになり、`CONSTRUCT` の結果を自分のストアへ読み込んだ時点でそれが利用者のオントロジーの一部になります（[ADR-0034](docs/adr/0034-construct-describe.md) 決定4 が切り詰めのために拒んだのと同じ理由です）。

**上限を超えて 413 になったときは警告しません。** 返していないものについて警告すると、受け取った側に対応するトリプルがありません。
...
- **`platform-admin` アプリロールが必要です([ADR-0014](docs/adr/0014-namespace-rbac.md) 決定2・3)。** 名前空間の作成と `POST /admin/reconcile` はこのロールを要求します。**`azd up` の `postdeploy` は同梱サンプルの名前空間を作るため、これが無いと 403 で止まります。**
  ...
  **人間側(`idtyp: "user"`)も実機で確認済みです**(2026-09-13。`P2A-18`、[ADR-0035 補記1](docs/adr/0035-actor-type.md))。v2.0 のアクセストークンに `idtyp: "user"` が乗ることを実測しました — Entra の文書は `include_user_token` を v1.0 固有の節に載せていますが、**実際には v2.0 でも効きます**。したがって人間の行為は `user` として記録され、`prov:Person` が出ます。**`idtyp` を設定していないテナントでは相変わらず付かない**ので、「クレームが無い」を「人間である」と読まない設計(3 値)は変えていません。
